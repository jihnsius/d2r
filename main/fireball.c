...
int red_flag_spawn;		// jinx 12-17-12 ctfc
int blue_flag_spawn;		// jinx 12-17-12 ctfc
					...
					if (object_to_object_visibility(obj, obj0p, FQ_TRANSWALL)) {
						...
						switch ( obj0p->type )	{
							...
							case OBJ_PLAYER:	{
								...
								if ( obj0p->shields >= 0 )
								{	
									//set_killer_weapon(objp->id, objp->type);		// jinx 02-05-13 weapon types
...
object *object_create_badass_explosion(object *objp, short segnum, vms_vector * position, fix size, int vclip_type, fix maxdamage, fix maxdistance, fix maxforce, int parent )
{
	...
	{
		if (!promod) create_smart_children(objp, NUM_SMART_CHILDREN);
		if (promod) create_smart_children(objp, NUM_SMART_CHILDREN_PROMOD);
	}
...
//--old-- 			segnum /= 2;
		...
		if (ctfc)	// jinx 01-11-13 ctfc: preparing to migrate all data over to specific gametype // probably a more efficient way to do this without wasting cycles on pick_random_point_in_seg if it's a flag and ctfc, but I don't want to risk breaking everything
		{
			if (powerup_type == POW_FLAG_BLUE) 
			{ 
				segnum = blue_flag_spawn; 
				new_pos = blue_flag_pos;
			} 
			if (powerup_type == POW_FLAG_RED) 
			{
				segnum = red_flag_spawn; 
				new_pos = red_flag_pos;
			}
		}
