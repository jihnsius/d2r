...
int do_flag_blink()	// jinx 01-04-13 ctfc: function to make flags blink; returns 1 for display flag, returns 0 for don't display flag
{
	int blink_timer = GameTime64/10000;
	if ( blink_timer % 2 == 0 )	// if even
		return 1;
	else	// if odd
		return 0;
}
...
void hud_show_flag(void)
{
	if (Game_mode & GM_CAPTURE)
	{
		...
		if (PlayerCfg.CockpitMode[1] == CM_FULL_COCKPIT) 
		{
			y=HUD_SCALE_Y_AR(GameBitmaps[ GET_GAUGE_INDEX(GAUGE_LIVES) ].bm_h)+FSPACY(1);
			x = FSPACX(2);
		}
		else if (PlayerCfg.CockpitMode[1] == CM_STATUS_BAR) 
		{
		...
		{
			y=HUD_SCALE_Y_AR(GameBitmaps[ GET_GAUGE_INDEX(GAUGE_LIVES) ].bm_h+GameBitmaps[ GET_GAUGE_INDEX(KEY_ICON_RED) ].bm_h)+FSPACY(7);
			x = FSPACX(2);
		}
		else
			Int3();		// 'your mother' is not a valid cockpit
		...
		if (ctfc)
		{
			...
			{
				if ((Players[i].flags & PLAYER_FLAGS_FLAG)  && (get_team(i) == get_team(Player_num)) && (i != Player_num) && do_flag_blink())	// jinx 01-15-13 ctfc: if a teammate has the opponent's flag, show the opponent's flag blinking
				{
					icon = (get_team(Player_num) == TEAM_BLUE)?FLAG_ICON_RED:FLAG_ICON_BLUE;
					bm = &GameBitmaps[ GET_GAUGE_INDEX(icon) ];
	...
	extern int segment_contains_object(int obj_type, int obj_id, int segnum);
	
	if (Game_mode & GM_CAPTURE && ctfc)	// jinx 01-04-13 ctfc: flag indicator blinks when your flag's not home
	{
		int x=0,y=0,icon;
		grs_bitmap *bm;
		...
		for (int i = 0;i<=Highest_segment_index+1;i++)	// display that your flag is home
		{
			if ((Segment2s[i].special == SEGMENT_IS_GOAL_BLUE) && ((segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, i) || (segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, blue_flag_spawn)))) && (get_team(Player_num)==TEAM_BLUE))		// if your flag is home, display it
			{
				icon = (get_team(Player_num) == TEAM_RED)?FLAG_ICON_RED:FLAG_ICON_BLUE;
				bm = &GameBitmaps[ GET_GAUGE_INDEX(icon) ];

				PAGE_IN_GAUGE( icon );
				x = grd_curcanv->cv_bitmap.bm_w; x-= HUD_SCALE_X_AR(bm->bm_w);
				
				hud_bitblt_free(x,y,HUD_SCALE_X_AR(bm->bm_w),HUD_SCALE_Y_AR(bm->bm_h),bm);
			} 
			if ((Segment2s[i].special == SEGMENT_IS_GOAL_BLUE) && (!(segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, i)) && !(segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, blue_flag_spawn))) && (get_team(Player_num)==TEAM_BLUE) && do_flag_blink())		// if your flag is not home, display it blinking
			{
				icon = (get_team(Player_num) == TEAM_RED)?FLAG_ICON_RED:FLAG_ICON_BLUE;
				bm = &GameBitmaps[ GET_GAUGE_INDEX(icon) ];
			...
			if ((Segment2s[i].special == SEGMENT_IS_GOAL_RED) && ((segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, i) || (segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, red_flag_spawn)))) && (get_team(Player_num)==TEAM_RED))		// if your flag is home, display it
			{
				icon = (get_team(Player_num) == TEAM_RED)?FLAG_ICON_RED:FLAG_ICON_BLUE;
				bm = &GameBitmaps[ GET_GAUGE_INDEX(icon) ];
			...
			if ((Segment2s[i].special == SEGMENT_IS_GOAL_RED) && (!(segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, i)) && !(segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, red_flag_spawn))) && (get_team(Player_num)==TEAM_RED) && do_flag_blink())		// if your flag is not home, display it blinking
			{
				icon = (get_team(Player_num) == TEAM_RED)?FLAG_ICON_RED:FLAG_ICON_BLUE;
				bm = &GameBitmaps[ GET_GAUGE_INDEX(icon) ];
...
void hud_show_weapons(void)
{
	...
	else
	{
		...
		if (!Netgame.Instagib) sprintf(weapon_str, "%s %d",weapon_name,Players[Player_num].secondary_ammo[Secondary_weapon]);
		if (Netgame.Instagib) 
		{
			if (Players[Player_num].net_kills_total > 0) sprintf(weapon_str, "acc%: %d",(int)((float)Players[Player_num].net_kills_total/(float)total_shots)*100.0);
			if (Players[Player_num].net_kills_total <= 0) sprintf(weapon_str, "acc%: 0");
		}
		gr_get_string_size(weapon_str, &w, &h, &aw );
...
void draw_weapon_info_sub(int info_index,gauge_box *box,int pic_x,int pic_y,char *name,int text_x,int text_y)
{
	...
	if (PlayerCfg.HudMode == 0)
	{
		...
		if (info_index == LASER_ID || info_index == SUPER_LASER_ID)
		{
			...
			if (Netgame.Instagib)
			{
				gr_printf(text_x,text_y+LINE_SPACING, "Reaper");
			}
		}
	}
}
...
void hud_show_kill_list()
{
	...
	for (i=0;i<n_players;i++) {
		...
		else if (Game_mode & GM_CAPTURE)
			gr_printf(x1,y,"%3d",Players[player_num].flag_points_total,Players[player_num].KillGoalCount);
...
void show_proxy_box()	// jinx 01-10-13 ctfc: draws indicator on all team proxies and smart mines (might expand this later to be variable to draw indicators on anything, like flags). 
{
	for (int objnum=0;objnum<=Highest_object_index;objnum++)
	{
		object		*probj = &Objects[objnum];
	
		// (if it's a weapon) and (it is in player's line of sight) and (it's a prox or smine) and (the owner of it is on the same team as me) then (draw it) otherwise gtfo
		
		if (((probj->type == OBJ_WEAPON) && (see_object(objnum)) &&  ((probj->id == PROXIMITY_ID) || (probj->id == SUPERPROX_ID))) && (get_team(probj->ctype.laser_info.parent_num) == get_team(Player_num)))
		{ 
			g3s_point player_point;
			g3_rotate_point(&player_point,&Objects[objnum].pos);
			...
			if (player_point.p3_codes == 0) //on screen
			{
				...
				if (!(player_point.p3_flags & PF_OVERFLOW))
				{
					...
					dy = -fixmuldiv(fixmul(Objects[objnum].size,Matrix_scale.y),i2f(grd_curcanv->cv_bitmap.bm_h)/2,player_point.p3_z);
					dx = fixmul(dy,grd_curscreen->sc_aspect);
					color_num = (Game_mode & GM_TEAM)?get_team(Player_num):Player_num;
					memset(&s, '\0', CALLSIGN_LEN+10);
...
/*					if (probj->id == PROXIMITY_ID)
						gr_setcolor(BM_XRGB(99,12,12));
					if (probj->id == SUPERPROX_ID)*/
						gr_setcolor(BM_XRGB(95,75,20));
			
					// draws a triangle, because squares are already done for flag carriers and the flag itself (i want to make this an equilateral triangle eventually. currently it's just isosceles)
					/*gr_line(x+dx-w,y-dy+(h/2),x+dx,y-dy+(h/2));		// 1
					gr_line(x+dx,y-dy+(h/2),x+dx-(w/2),y-dy+(h*1.5));		// 2
					gr_line(x-dx,y-dy+(h/2),x-dx+w,y-dy+(h/2));		// 3
					gr_line(x-dx,y-dy+(h/2),x-dx+(w/2),y-dy+(h*1.5));		// 4
					gr_line(x,y+dy+(h/2),x-(w/2),y+dy-(h/2));	// 5
					gr_line(x,y+dy+(h/2),x+(w/2),y+dy-(h/2));		// 6*/
					
					// 2,400,000 max, 0 min
					float life = probj->lifeleft;
					float max = 0;
					int thickness = 2;
					...
					float xxx = life/max;
					float xx1 = (x+dx-(w/4));
					float xx2 = ((-dx*2)+(w/2))*xxx;

					gr_line(x-dx,y-dy,x-dx+w,y-dy);	// bottom right horizontal
					gr_line(x-dx,y-dy,x-dx,y-dy+h);	// bottom right vertical
					gr_line(x+dx-w,y+dy,x+dx,y+dy);// top left horizontal
					gr_line(x+dx,y+dy,x+dx,y+dy-h);	// top left vertical
						
					for (int s =0;s<=thickness;s++)
					{
						int i = s * 50000;
						gr_line(x+dx-(w/4),y+dy-(h/4)+i,xx1+xx2,y+dy-(h/4)+i);		// life bar
					}
				}
			}
		}
...
void show_flag_indicator()	// jinx 01-10-13 ctfc: draws indicator on your flag through walls
{
	if (!(Game_mode & GM_CAPTURE)) return;
	int red, green, blue;
	red = green = blue = 0;
	for (int objnum=0;objnum<=Highest_object_index;objnum++)
	{
		...
		// (if it's a powerup) and (if it's red flag and I'm on red team) or (it's blue flag and i'm on blue team) then (draw it) otherwise gtfo
		
		if ((probj->type == OBJ_POWERUP) && (((probj->id == POW_FLAG_RED) && (get_team(Player_num)==TEAM_RED)) || ((probj->id == POW_FLAG_BLUE) && (get_team(Player_num)==TEAM_BLUE))))
		{ 
			g3s_point player_point;
			g3_rotate_point(&player_point,&Objects[objnum].pos);
			...
			if (player_point.p3_codes == 0) //on screen
			{
				...
				if (!(player_point.p3_flags & PF_OVERFLOW))
				{
					...
					h = dy/2;
					int thickness = 2;
					float max = Netgame.AutoReturnTime*5;
					float life = 0;
					if (probj->id == POW_FLAG_RED)
						life = Netgame.AutoReturnTime*5 - red_flag_respawn_countdown + 1;
					if (probj->id == POW_FLAG_BLUE)
						life = Netgame.AutoReturnTime*5 - blue_flag_respawn_countdown + 1;
						
					if (life > Netgame.AutoReturnTime*5) life = Netgame.AutoReturnTime*5;
					if (Netgame.AutoReturnTime == 0) { life = 1; max = 1; }
					
					float xxx = life/max;
					float xx1 = (x+dx-(w/4));
					float xx2 = ((-dx*2)+(w/2))*xxx;
					//HUD_init_message(HM_MULTI, "%f", life);
					
/*					if (Game_mode & GM_CAPTURE)
						gr_setcolor((get_team(Player_num) == TEAM_RED)?BM_XRGB(31,0,0):BM_XRGB(0,0,31));*/
						float sinewave = sin(GameTime64/5000);
						if (get_team(Player_num)==TEAM_RED)
						{
							green = blue = (sinewave+1) * 50 - 1;
							red = 99;
						}	
						if (get_team(Player_num)==TEAM_BLUE)
						{
							green = red = (sinewave+1) * 50 - 1;
							blue = 99;
						}
						// green = 100-((sin(GameTime64/5000)+1) * 50);
						
						
						//HUD_init_message(HM_MULTI, "%d", green);
						gr_setcolor(BM_XRGB(red,green,blue));
					...
					for (int s =0;s<thickness;s++)
					{
						int i = s * 130000;
						int j = s * 50000;
						gr_line(x+dx-w,y-dy+i,x+dx,y-dy+i); // 1
						gr_line(x+dx-i,y-dy,x+dx-i,y-dy+h); // 2
						gr_line(x-dx,y+dy-i,x-dx+w,y+dy-i); // 7
						gr_line(x-dx+i,y+dy,x-dx+i,y+dy-h);  // 8
						gr_line(x+dx-(w/4),y-dy+(h/4)-j,xx1+xx2,y-dy+(h/4)-j);		// life bar
					}
				}
			}
		}
...
void instagun_tracer()	// jinx 01-10-13 ctfc: draws indicator on your flag through walls
{
	for (int objnum=0;objnum<=Highest_object_index;objnum++)
	{
		object		*probj = &Objects[objnum];
		...
		// (if it's a powerup) and (if it's red flag and I'm on red team) or (it's blue flag and i'm on blue team) then (draw it) otherwise gtfo
		if ((probj->id == bolt_type) && (see_object(objnum))) 
		{ 
			g3s_point player_point;
			g3_rotate_point(&player_point,&Objects[objnum].pos);
			...
			if (player_point.p3_codes == 0) //on screen
			{
				...
				if (!(player_point.p3_flags & PF_OVERFLOW))
				{
					...
					h = dy/2;
						...
						gr_setcolor(BM_XRGB(99,99,99));

					// draw the lines
					gr_line(x+dx-w,y-dy,x+dx,y-dy); // 1
...
void draw_hud()
{
	...
	if ( !Rear_view && PlayerCfg.CockpitMode[1]!=CM_REAR_VIEW && PlayerCfg.CockpitMode[1]!=CM_STATUS_BAR/* && !(Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)*/) {	// jinx 01-28-13 spec
		hud_show_score();
		if (score_time)
			hud_show_score_added();
	}

	if ( !Rear_view && PlayerCfg.CockpitMode[1]!=CM_REAR_VIEW/* && !(Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)*/)	// jinx 01-28-13 spec
	...
	if (!Rear_view && PlayerCfg.CockpitMode[1]!=CM_REAR_VIEW)
	{
		...
		if (Netgame.BombIndicator)
			show_proxy_box();	// jinx 01-10-13 ctfc: will work in any gametype, really. helps distinguish your own/team bombs from others
			
		if ((Game_mode & GM_CAPTURE) && (Netgame.FlagIndicator))
			show_flag_indicator();	// jinx 01-10-13 ctfc

		//if (Netgame.Instagib)
			//instagun_tracer();	// jinx 01-20-13 instagib
		...
		if (PlayerCfg.CockpitMode[1]!=CM_STATUS_BAR && !(Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)) // jinx 01-28-13 spec
			hud_show_lives();
		if (Game_mode&GM_MULTI && Show_kill_list)	// jinx 01-28-13 spec
			hud_show_kill_list();
...
void ctfc_fontcolor()
{
	if (Game_mode & GM_CAPTURE && ctfc)
	{
		int color = (Game_mode & GM_TEAM)?get_team(Player_num):Player_num;
		gr_set_fontcolor( BM_XRGB( player_rgb[color].r, player_rgb[color].g, player_rgb[color].b ), -1 );
	}
}
