...
void init_player_stats_game(ubyte pnum)
{
	...
	Players[pnum].flag_returns_total = 0;		// jinx 01-03-13 ctfc
	Players[pnum].flag_captures_total = 0;
	Players[pnum].net_teamkills_total = 0;
	Players[pnum].flag_points_total = 0;
		
	Players[pnum].spec_flags &= ~PLAYER_FLAGS_SPECTATING;	// jinx 02-01-13 spec
	Players[pnum].spec_flags &= ~PLAYER_FLAGS_SPECTATING_ME;
	if (misc)
	{
		last_killed = 0;		// jinx 01-22-13 misc
		killstreak = 0;
...
void init_ammo_and_energy(void)
{
	if (misc)
	{
		last_killed = 0;		// jinx 01-22-13 instagib
		killstreak = 0;
		highest_killstreak = 0;
	}
...
void init_player_stats_new_ship(ubyte pnum)
{
	...
	Objects[Players[pnum].objnum].flags &= ~(OF_SHOULD_BE_DEAD|OF_EXPLODING);			// don't apply these to the player as they're only flags for objects
	
	if (spec) init_all_spectator_stuff();	// jinx 02-04-13 spec
}
...
void instagib_main()	// jinx 01-22-13 instagib
{
	//HUD_init_message(HM_MULTI, "%d >= %d", gib_time, current_time - gib_timer);
	//HUD_init_message(HM_MULTI, "%d <= %d", last_killed, current_time - last_killed_minimum);
}


void do_flag_resync_stuff()
{
/*	int r = 0;
	int b = 0;
	int i = 0;
	int total_red_flags = 0;
	int total_blue_flags = 0;
	if (!(multi_i_am_master())) return;
	for (int s = 0;s<=Highest_object_index+10;s++)
	{
		if (Objects[s].id == POW_FLAG_RED) 
		{
			total_red_flags++;
	...
	for (int s = 0;s<=Highest_object_index+10;s++)
	{
		if (Objects[s].id == POW_FLAG_BLUE) 
		{
			Ctfc_flag[i].object_num = s;
			i++;
			total_flags++;
		}
	}
	for (int i = 0; i <= Highest_object_index; i++)
	{
		if (Objects[i].id == POW_FLAG_RED || Objects[i].id == POW_FLAG_BLUE)
			HUD_init_message(HM_MULTI, "flag objnum: %d", i);
	}*/
}



void flag_classic_main()		// jinx 01-06-13 ctfc: main loop for ctfc (timers and stuff)
{
	Assert(Game_mode & GM_CAPTURE);
		
	if (!(multi_i_am_master()))		// jinx 01-12-13 ctfc: non host players need to keep track of flag return timers for the flag indicators // probably merge this with the code just below this for optimization purposes
	{
		for (int r = 0;r<=N_players;r++)
		{
			for (int i = 0;i<=Highest_segment_index+1;i++)
			{
				if (((get_team(r)==TEAM_BLUE) && (Players[r].flags & PLAYER_FLAGS_FLAG) && (Players[r].shields >= 0)) || ((Segment2s[i].special == SEGMENT_IS_GOAL_RED) && (segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, i) || (segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, red_flag_spawn)))))
				{
					red_flag_timer = 0;
					red_flag_respawn_countdown = 0; 
					break;
				}
			}
			if ((r==N_players) && (red_flag_timer !=  current_time))
			{
				red_flag_timer = current_time;
				red_flag_respawn_countdown += 1;
		...
		for (int b = 0;b<=N_players;b++)
		{
			for (int i = 0;i<=Highest_segment_index+1;i++)
			{
				if (((get_team(b)==TEAM_RED) && (Players[b].flags & PLAYER_FLAGS_FLAG) && (Players[b].shields >=0)) || ((Segment2s[i].special == SEGMENT_IS_GOAL_BLUE) && (segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, i) || (segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, blue_flag_spawn)))))
				{
					blue_flag_timer = 0; 
					blue_flag_respawn_countdown = 0; 
					break;
				}
			}
			if ((b==N_players) && (blue_flag_timer !=  current_time))
			{
				blue_flag_timer = current_time;
				blue_flag_respawn_countdown += 1;
			}
		}
	}
	
	if (!(multi_i_am_master())) return;		// jinx 01-06-13 ctfc: anything after this point is host-side only
	
	do_flag_resync_stuff();
	
	if (multi_i_am_master() && (Netgame.AutoReturnTime != 0))
	{
		for (int r = 0;r<=N_players;r++)
		{
			for (int i = 0;i<=Highest_segment_index+1;i++)
			...
			if (red_flag_respawn_countdown >= ((Netgame.AutoReturnTime*5)+1))
			{
				red_flag_respawn_countdown = 0;
				red_flag_timer = 0;
				multi_send_autoreturn_red();		// flag you're drunk, go home
				for (int i = 0;i<=Highest_object_index+10;i++)
				{
					if (Objects[i].id == POW_FLAG_RED)
					{
						Objects[i].flags |= OF_SHOULD_BE_DEAD;
						multi_send_remobj(i);
						multi_send_flag_disappearance(&Objects[i]);
					}
				}
		...
		for (int b = 0;b<=N_players;b++)
		{
			...
			if (blue_flag_respawn_countdown >= ((Netgame.AutoReturnTime*5)+1))
			{
				blue_flag_respawn_countdown = 0;
				blue_flag_timer = 0;
				multi_send_autoreturn_blue();		// flag you're drunk, go home
				for (int i = 0;i<=Highest_object_index+10;i++)
				{
...
void do_last_killed()		// jinx 01-28-13 misc
{
	if (last_killed_timer != current_time)
	{
		last_killed_timer = current_time;
		last_killed += 1;
	}
}
...
void update_player_stats()
{
		...
		current_time = f2i(Players[Player_num].time_level);
		
		if (Game_mode & GM_CAPTURE)
			flag_classic_main();
			
		if (Netgame.Instagib) instagib_main();
...
void StartNewGame(int start_level)
{
	...
	promod_first_time = 0;		// jinx 02-16-13 promod
	d1_first_time = 0;
}
...
void bash_to_shield (int i,char *s)
{
	...
	if (!bash_to_first(i, s))
	{
		Objects[i].id = POW_SHIELD_BOOST;
		HUD_init_message(HM_CONSOLE, "bashing %s to shield", s);
	}
...
int bash_to_first (int i,char *s)
{

	if (!Netgame.Descent1) return 0;
#ifdef NETWORK
	int type=Objects[i].id;
#endif
	...
	switch (type)
	{
		case POW_AFTERBURNER:
			HUD_init_message(HM_CONSOLE, "bashing afterburner to obj_none");
			Objects[i].type = OBJ_NONE; return_with =  1; break;
		case POW_HEADLIGHT:
			HUD_init_message(HM_CONSOLE, "bashing headlight to obj_none");
			Objects[i].type = OBJ_NONE; return_with =  1; break;
		case POW_CONVERTER:
			HUD_init_message(HM_CONSOLE, "bashing energy converter to obj_none");
			Objects[i].type = OBJ_NONE; return_with =  1; break;
		case POW_GAUSS_WEAPON:
			HUD_init_message(HM_CONSOLE, "bashing gauss to vulcan");
			Objects[i].id = POW_VULCAN_WEAPON; return_with =  1; break;
		case POW_HELIX_WEAPON:
			HUD_init_message(HM_CONSOLE, "bashing helix to spread");
			Objects[i].id = POW_SPREADFIRE_WEAPON; return_with =  1; break;
		case POW_PHOENIX_WEAPON:
			HUD_init_message(HM_CONSOLE, "bashing helix to spread");
			Objects[i].id = POW_SPREADFIRE_WEAPON; return_with =  1; break;
		case POW_OMEGA_WEAPON:
			HUD_init_message(HM_CONSOLE, "bashing omega to fusion");
			Objects[i].id = POW_FUSION_WEAPON; return_with =  1; break;
		case POW_SMISSILE1_1:
			HUD_init_message(HM_CONSOLE, "bashing flash to conc");
			Objects[i].id = POW_MISSILE_1; return_with =  1; break;
		case POW_SMISSILE1_4:
			HUD_init_message(HM_CONSOLE, "bashing flash 4 pack to conc 4 pack");
			Objects[i].id = POW_MISSILE_4; return_with =  1; break;
		case POW_GUIDED_MISSILE_1:
			HUD_init_message(HM_CONSOLE, "bashing guided to homer");
			Objects[i].id = POW_HOMING_AMMO_1; return_with =  1; break;
		case POW_GUIDED_MISSILE_4:
			HUD_init_message(HM_CONSOLE, "bashing guided 4 pack to homer 4 pack");
			Objects[i].id = POW_HOMING_AMMO_4; return_with =  1; break;
		case POW_SMART_MINE:
			HUD_init_message(HM_CONSOLE, "bashing smine to proxy ");
			Objects[i].id = POW_PROXIMITY_WEAPON; return_with =  1; break;
		case POW_MERCURY_MISSILE_1:
		case POW_MERCURY_MISSILE_4:
			HUD_init_message(HM_CONSOLE, "bashing merc and merc 4 pack to smart");
			Objects[i].id = POW_SMARTBOMB_WEAPON; return_with =  1; break;
		case POW_EARTHSHAKER_MISSILE:
			HUD_init_message(HM_CONSOLE, "bashing earthshaker to mega");
			Objects[i].id = POW_MEGA_WEAPON; return_with =  1; break;
	}
	if (return_with == 1)
	{
		Objects[i].rtype.vclip_info.vclip_num = Powerup_info[Objects[i].id].vclip_num;
...
void InitPlayerPosition(int random_flag)
{
	...
	if ((Game_mode & GM_CAPTURE) || assault)		// jinx 12-16-12 ctfc players spawn on their corresponding sides (red is odd, blue is even)
	{
		first_spawns = 1;
		//NewPlayer = d_rand() % NumNetPlayerPositions;
		if (get_team(Player_num)==TEAM_BLUE)	// blue 
		{
			if ((NewPlayer == 1) || (NewPlayer == 3) || (NewPlayer == 5)) NewPlayer++;
			if (NewPlayer == 7) NewPlayer = 0;
		}
		if (get_team(Player_num)==TEAM_RED)	// red
		{
			if ((NewPlayer == 0) || (NewPlayer == 2) || (NewPlayer == 4) || (NewPlayer == 6)) NewPlayer++;
