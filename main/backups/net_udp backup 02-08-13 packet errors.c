...
void net_udp_send_game_info(struct _sockaddr sender_addr, ubyte info_upid)
{
	...
	else
	{
		...
		PUT_INTEL_INT(buf + len, Netgame.RespawnTime);			len += 4;
		PUT_INTEL_INT(buf + len, Netgame.FlagInHome);			len += 4;
		PUT_INTEL_INT(buf + len, Netgame.FlagIndicator);			len += 4;
		PUT_INTEL_INT(buf + len, Netgame.BombIndicator);		len += 4;
		PUT_INTEL_INT(buf + len, Netgame.Instagib);					len += 4;
		PUT_INTEL_INT(buf + len,  Netgame.AutoReturnTime);		len += 4;
		PUT_INTEL_INT(buf + len, Netgame.Descent4);						len += 4;		// jinx 02-02-13 d4
...
void net_udp_process_game_info(ubyte *data, int data_len, struct _sockaddr game_addr, int lite_info)
{
	...
	else
	{
		...
		Netgame.RespawnTime = GET_INTEL_INT(&(data[len]));			len += 4;
		Netgame.FlagInHome = GET_INTEL_INT(&(data[len]));				len += 4;
		Netgame.FlagIndicator = GET_INTEL_INT(&(data[len]));			len += 4;
		Netgame.BombIndicator = GET_INTEL_INT(&(data[len]));			len += 4;
		Netgame.Instagib = GET_INTEL_INT(&(data[len]));					len += 4;
		Netgame.AutoReturnTime = GET_INTEL_INT(&(data[len]));		len += 4;
		Netgame.Descent4 = GET_INTEL_INT(&(data[len]));						len += 4;		// jinx 02-02-13 d4
...
static int opt_AutoReturnTime,opt_RespawnTime,opt_FlagInHome,opt_FlagIndicator,opt_BombIndicator,opt_d4,opt_Instagib;
...
{
	int opt=0,i;
	char PlayText[80],KillText[80],srinvul[50],packstring[5],AutoReturnTimeText[80],RespawnTimeText[80];
	
	int total_max_menu_options = 25;
	
#ifndef USE_TRACKER
	total_max_menu_options--;
#endif
	if (!D4Equipped())
		total_max_menu_options--;
		
	newmenu_item m[total_max_menu_options];
	...
	{
		opt_d4=opt;
		m[opt].type = NM_TYPE_CHECK; m[opt].text = "D1 style weapons"; m[opt].value=Netgame.Descent4; opt++;
	}

	opt_Instagib=opt;
	m[opt].type = NM_TYPE_CHECK; m[opt].text = "Instagib"; m[opt].value=Netgame.Instagib; opt++;		// jinx 02-06-13 instagib
	
	opt_AutoReturnTime=opt;
	sprintf( AutoReturnTimeText, "Flag Autoreturn: %d secs", Netgame.AutoReturnTime*5);
	m[opt].type = NM_TYPE_SLIDER; m[opt].value=Netgame.AutoReturnTime; m[opt].text= AutoReturnTimeText; m[opt].min_value=0; m[opt].max_value=8; opt++;		// jinx 12-24-12 ctfc

	opt_RespawnTime=opt;
	sprintf( RespawnTimeText, "Respawn: %d secs", Netgame.RespawnTime*5);
	m[opt].type = NM_TYPE_SLIDER; m[opt].value=Netgame.RespawnTime; m[opt].text= RespawnTimeText; m[opt].min_value=0; m[opt].max_value=6; opt++;				// jinx 12-24-12 ctfc
	
	opt_FlagInHome=opt;
	m[opt].type = NM_TYPE_CHECK; m[opt].text = "Flag must be home to score"; m[opt].value=Netgame.FlagInHome; opt++;															// jinx 12-24-12 ctfc
	
	opt_FlagIndicator=opt;
	m[opt].type = NM_TYPE_CHECK; m[opt].text = "Draw indicator around flag"; m[opt].value=Netgame.FlagIndicator; opt++;															// jinx 12-24-12 ctfc
	
	opt_BombIndicator=opt;
	m[opt].type = NM_TYPE_CHECK; m[opt].text = "Draw indicator around bombs"; m[opt].value=Netgame.BombIndicator; opt++;															// jinx 12-24-12 ctfc
...
menu:
	...
	Netgame.FlagInHome=m[opt_FlagInHome].value;
	Netgame.FlagIndicator=m[opt_FlagIndicator].value;
	Netgame.BombIndicator=m[opt_BombIndicator].value;
	Netgame.Instagib=m[opt_Instagib].value;	// jinx 02-06-13 instagib
	Netgame.Descent4=m[opt_d4].value;		// jinx 02-02-13 d4
...
int net_udp_more_options_handler( newmenu *menu, d_event *event, void *userdata )
{
	...
	switch (event->type)
	{
		case EVENT_NEWMENU_CHANGED:
			...
			else if (citem == opt_AutoReturnTime)
			{
				if (!(Game_mode & GM_CAPTURE))
				{
					nm_messagebox ("Sorry",1,TXT_OK,"these are only for ctf!");
					menus[opt_AutoReturnTime].value=4;
					return 0;
				}
				Netgame.AutoReturnTime=menus[opt_AutoReturnTime].value;
				sprintf( menus[opt_AutoReturnTime].text, "Flag AutoReturn: %d secs", Netgame.AutoReturnTime*5);
			}
			else if (citem == opt_RespawnTime)
			{
				if (!(Game_mode & GM_CAPTURE))
				{
					nm_messagebox ("Sorry",1,TXT_OK,"these are only for ctf!");
					menus[opt_RespawnTime].value=1;
					return 0;
				}
				Netgame.RespawnTime= menus[opt_RespawnTime].value;
				sprintf( menus[opt_RespawnTime].text, "Respawn: %d secs", Netgame.RespawnTime*5);
			}
			break;
	...
	switch (event->type)
	{
		...
		case EVENT_NEWMENU_SELECTED:
			...
			if (citem==opt->moreopts)
			{
				...
				if (menus[opt->capture].value)
					Game_mode=GM_CAPTURE;		// jinx 12-25-12 ctfc
...
int net_udp_setup_game()
{
	...
	Netgame.RespawnTime = 1;
	Netgame.AutoReturnTime = 3;
	Netgame.FlagInHome = 1;
	Netgame.BombIndicator = 1;
	Netgame.FlagIndicator = 0;
	Netgame.Instagib = 0;
	Netgame.Descent4 = 0;
	...
	if (!Network_rejoined)
	{
		...
		for (i=0; i<NumNetPlayerPositions; i++)
		{
			if (Game_mode & GM_CAPTURE)
			{
				if (first_spawns)
					NewPlayer = i;			
				if (get_team(Player_num)==TEAM_BLUE)	// jinx ctfc blue 
				{
					if ((i == 1) || (i == 3) || (i == 5)) NewPlayer = i + 1;
					if (i == 7) NewPlayer = 0;
				}
				if (get_team(Player_num)==TEAM_RED)	// jinx ctfc red
				{
					if ((i == 0) || (i == 2) || (i == 4) || (i == 6)) NewPlayer = i + 1;
				}
	
				Objects[Players[i].objnum].pos = Player_init[Netgame.locations[NewPlayer]].pos;
				Objects[Players[i].objnum].orient = Player_init[Netgame.locations[NewPlayer]].orient;
				obj_relink(Players[i].objnum,Player_init[Netgame.locations[NewPlayer]].segnum);
			}
			if (!(Game_mode & GM_CAPTURE))
			{
...
int net_udp_do_join_game()
{
	...
	if ( !D4Equipped() && (Netgame.Descent4) )
	{
		nm_messagebox(TXT_SORRY, 1, TXT_OK, "descent4.ham not installed.");		// jinx 02-04-13 d4
		return 0;
	}
