...
void multi_compute_kill(int killer, int killed)
{
	...
	if (instagib)
	{
		...
		if ((killer_pnum==Player_num) && (killstreak % 5 == 0)) 
		{		
		...
		if ((killed_pnum==Player_num) && ((current_time - last_killed) >= 90)) 
		{ 
			multi_send_last_killed (last_killed); 
			last_killed = current_time;
		}
	}
}
...
void multi_do_last_killed (char * buf)
{
	...
	int secs = (current_time - lastkilled) % 60;
	int mins = (current_time - lastkilled) / 60;
