...
void state_object_to_object_rw(object *obj, object_rw *obj_rw)
{
	...
	switch (obj_rw->render_type)
	{
		...
		{
			int i;
			if (obj->render_type == RT_NONE && obj->type != OBJ_GHOST && obj->type != OBJ_SPECTATOR) // HACK: when a player is dead or not connected yet, clients still expect to get polyobj data - even if render_type == RT_NONE at this time. Here it's not important, but it might be for Multiplayer Savegames.
