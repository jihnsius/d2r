...
// end of section to be moved.
	...
	if (((obj->type!=OBJ_PLAYER) || (obj->id!=Player_num)) && ((Players[Player_num].is_spectating != 1))) return;	//references to player_ship require that this obj be the player
	...
	if ((Players[Player_num].flags & PLAYER_FLAGS_AFTERBURNER && (Players[Player_num].is_spectating != 1)) || (Players[Player_num].is_spectating))
	{
		if (Controls.afterburner_state) {			//player has key down
			//if (forward_thrust_time >= 0) { 		//..and isn't moving backward
			{
				...
				if (Players[Player_num].is_spectating)	forward_thrust_time *= 1.5;	// jinx 01-25-13 spec
	
				old_count = (Afterburner_charge / (DROP_DELTA_TIME/AFTERBURNER_USE_SECS));

				if ((Players[Player_num].is_spectating != 1))		// jinx 01-25-13 spec
