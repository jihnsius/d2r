...
int create_weapon_object(int weapon_type,int segnum,vms_vector *position)
{
	...
	if (instagib && (weapon_type == bolt_type)) // if instagib and not flare and not reactor blob
	{	
...
void Laser_player_fire_spread_delay(object *obj, int laser_type, int gun_num, fix spreadr, fix spreadu, fix delay_time, int make_sound, int harmless)
{
	...
	if (instagib && (weapon_type != FLARE_ID) && (weapon_type != 6))
	{	
		Weapon_info[weapon_type].persistent = 1;
		Weapon_info[weapon_type].fire_count = 1;
		Weapon_info[weapon_type].ammo_usage = 0;
...
void Laser_do_weapon_sequence(object *obj)
{
	...
	// jinx 01-20-13 instagib: placeholder
	/*if (((GameTime64/100000 % 10 == 0)))
	{
			create_weapon_object(FLARE_ID, obj->segnum, &obj->pos);
	}*/
...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	switch (weapon_num) {
		case LASER_INDEX: {
			...
			if (instagib)	// jinx 01-23-13 instagib
			{
				Laser_player_fire( objp, weapon_num, 0, 1, 0);
				total_shots++;
