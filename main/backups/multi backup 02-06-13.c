...
static const int message_length[MULTI_MAX_TYPE+1] = {
	...
	7, // MULTI_KILL_HOST
	5, // MULTI_KILL_CLIENT
	2, // MULTI_RETURN_BONUS
	9, // MULTI_DO_FLAG_SPAWNS
	1, // MULTI_REQUEST_FLAG_SPAWNS
	1, // MULTI_AUTORETURN_RED
	1, // MULTI_AUTORETURN_BLUE
	3, // MULTI_SEND_RETURN_FLAG
	2, // MULTI_SEND_CAPTURE_FLAG
	1, // MULTI_DO_DENIED
	4, // MULTI_DO_UPDATE_FLAG_STATS
	2, // MULTI_DO_FLAG_DROP
	2, // DO_FLAG_DISAPPEAR
	3, // MULTI_DO_KILLSTREAK
	3, // MULTI_DO_LAST_KILLED
	3, // MULTI_DO_SPEC_STATUS
	3, // REMAKE_PIGGY_INVISIBLE
	3, // DO_I_AM_NO_LONGER_SPECTATING_YOU
	3, // DO_I_AM_SPECTATING_YOU
...
multi_new_game(void)
{
	...
	for (i = 0; i < MAX_PLAYERS; i++)
	{
		...
		Players[i].flag_returns_total = 0;
		Players[i].flag_denials_total = 0;
		Players[i].flag_captures_total = 0;
		Players[i].flag_points_total = 0;
		Players[i].net_teamkills_total = 0;
	}
...
void multi_compute_kill(int killer, int killed)
{
	...
	if (ignore_last_kill_text)
		digi_play_sample( 52, F3_0 );
	else
	...
	else
	{
		if (!(Game_mode & GM_HOARD))
		{
			if (Game_mode & GM_TEAM)
			{
				...
				{
					team_kills[get_team(killed_pnum)] -= 1;
					Players[(int)killer_pnum].net_teamkills_total += 1;		// jinx 01-05-13 ctfc
				}
				else
			...
			else if ((get_team(killed_pnum) != get_team(killer_pnum)) && (Game_mode & GM_TEAM))
			{
				Players[killer_pnum].flag_points_total += 1;
	...
	if (misc)
	{
		if ((killer_pnum==Player_num) && (killer_pnum != killed_pnum)) 
		{ 
			killstreak++; 
		}
		if (killed_pnum == Player_num) 
		...
		if ((killer_pnum==Player_num) && ((killstreak % 5 == 0) && (killstreak != 0))) 
		{		
			multi_send_killstreak (killstreak); 
		}
		if (killstreak > highest_killstreak) 
		{
			highest_killstreak = killstreak;
		}
		if ((killed_pnum==Player_num) && (last_killed >= 90)) 
		{ 
			multi_send_last_killed (last_killed); 
			last_killed = 0;
		}
	}
...
multi_leave_game(void)
{
	...
	if (Game_mode & GM_NETWORK)
	{
		Players[Player_num].is_spectating = 0;		// jinx 01-26-13 spec
		Net_create_loc = 0;
...
void multi_send_message_end()
{
	...
	else if (!d_strnicmp (Network_message,"/m: ",4))
	{
		if ((Game_mode & GM_NETWORK) && (Game_mode & GM_TEAM))
		{
			int name_index=4;
			if (strlen(Network_message) > 4)
				while (Network_message[name_index] == ' ')
					name_index++;
	...
	else if (!d_strnicmp (Network_message,"/kill",5) && (Game_mode & GM_NETWORK) && !Control_center_destroyed)
	{
		console_kill = 1;
	...
	else if (!d_strnicmp (Network_message,"/spectate",9) && (Game_mode & GM_NETWORK) && !Control_center_destroyed && spec)		// jinx 02-04-13 spec
	{
...
multi_do_death(int objnum)
{
	...
	player_spawn_timer = f2i(Players[Player_num].time_level);		// jinx 12-23-12 ctfc
}
...
multi_do_kill(char *buf)
{
	...
	first_spawns = 0;	// jinx 01-05-13 ctfc: no longer spawning players in their player number slots
}
...
multi_do_remobj(char *buf)
{
	...
	if (Objects[local_objnum].id == (POW_FLAG_RED || POW_FLAG_BLUE ))  object_create_explosion(Objects[local_objnum].segnum, &Objects[local_objnum].pos, F1_0*7/2, VCLIP_POWERUP_DISAPPEARANCE );
...
void multi_disconnect_player(int pnum)
{
	...
	Players[pnum].is_spectating = 0;	// jinx 01-28-13 spec
	Players[pnum].flags &= ~PLAYER_FLAGS_SPECTATING;
	Players[pnum].flags &= ~PLAYER_FLAGS_SPECTATING_ME;
}
...
multi_send_player_explode(char type)
{
	...
	if ((current_time - last_killed) >= 90) { multi_send_last_killed (last_killed); }
	...
	{
		multi_send_flag_drop(Player_num);
	...
	killstreak = 0;
	if (last_killed >= 90) { multi_send_last_killed (last_killed); }
	last_killed = 0;
}
...
void multi_powcap_cap_objects()
{
	...
	if (Game_mode & GM_CAPTURE)
	{
		if (Players[Player_num].flags & PLAYER_FLAGS_FLAG)
		{
			...
			if (PowerupsInMine[(int)flagtype]+1 > MaxPowerupsAllowed[(int)flagtype]) // jinx: this appears to be what makes sure there's only one flag in the game for each team. 11-21-12
				Players[Player_num].flags&=(~PLAYER_FLAGS_FLAG);
		}
	}
...
multi_send_kill(int objnum)
{
	...
	first_spawns = 0;	// jinx 01-05-13 ctfc: no longer spawning players in their player number slot
}
...
multi_send_remobj(int objnum)
{
	...
	if ((Game_mode & GM_CAPTURE) && (Objects[objnum].id == (POW_FLAG_RED || POW_FLAG_BLUE))) object_create_explosion(Objects[objnum].segnum, &Objects[objnum].pos, F1_0*7/2, VCLIP_POWERUP_DISAPPEARANCE );
...
void multi_prep_level(void)
{
	...
	for (i=0; i<=Highest_object_index; i++)
	{
		if (instagib && (Objects[i].type == OBJ_POWERUP) && (!(Objects[i].id == POW_AFTERBURNER) && !(Objects[i].id == POW_CLOAK) && !(Objects[i].id == POW_INVULNERABILITY) && !(Objects[i].id == POW_HEADLIGHT) && !(Objects[i].id == POW_FLAG_RED) && !(Objects[i].id == POW_FLAG_BLUE)))
		{
		...
		if (Objects[i].type == OBJ_POWERUP)
		{
			...
			set_flag_home();	// jinx 12-17-12 ctfc
		}
	}
...
void multi_do_return_bonus(char *buf)
{
	char pnum=buf[1];
	int TheGoal;
	
	if (pnum==Player_num)
		HUD_init_message(HM_MULTI, "Flag returned!");
	else if (get_team(pnum)==TEAM_RED)
		HUD_init_message(HM_MULTI, "%s returned the red flag!",Players[(int)pnum].callsign);
	else if (get_team(pnum)==TEAM_BLUE)
		HUD_init_message(HM_MULTI, "%s returned the blue flag!",Players[(int)pnum].callsign);
		
	digi_play_sample (51,F1_0*2);
	
	team_kills[get_team(pnum)] += 1;
	Players[(int)pnum].flag_points_total += 1;
	Players[(int)pnum].KillGoalCount += 1;
	Players[(int)pnum].flag_returns_total += 1;
...
{
	multibuf[0] = MULTI_REQUEST_FLAG_SPAWNS;
	multi_send_data(multibuf, 1, 2);
	//multi_send_data_direct((ubyte*)multibuf, 1, multi_who_is_master(), 2);
}
...
void multi_send_return_bonus (char pnum)
{
	...
	multibuf[0]=MULTI_RETURN_BONUS;
	multibuf[1]=pnum;

	multi_send_data (multibuf,2,2);
	multi_do_return_bonus (multibuf);
}


void multi_do_autoreturn_red()		// jinx 12-23-12 ctfc
{
	HUD_init_message(HM_MULTI, "the red flag has autoreturned!");
	digi_play_sample (51,F1_0*2);
}
void multi_do_autoreturn_blue()		// jinx 12-23-12 ctfc
{
	HUD_init_message(HM_MULTI, "the blue flag has autoreturned!");
	digi_play_sample (51,F1_0*2);
}
void multi_send_autoreturn_red ()		// jinx 12-23-12 ctfc
{
	Assert (Game_mode & GM_CAPTURE);

	multibuf[0]=MULTI_AUTORETURN_RED;
	
	multi_send_data (multibuf,1,2);
	multi_do_autoreturn_red();
}
void multi_send_autoreturn_blue ()		// jinx 12-23-12 ctfc
{
	Assert (Game_mode & GM_CAPTURE);
	
	multibuf[0]=MULTI_AUTORETURN_BLUE;
	
	multi_send_data (multibuf,1,2);
	multi_do_autoreturn_blue();
}
...
void multi_do_capture_bonus(char *buf)
{
	...
	Players[(int)pnum].flag_points_total += 5;
	Players[(int)pnum].KillGoalCount+=5;
	Players[(int)pnum].flag_captures_total += 1;	// jinx 01-03-13 ctfc
...
//	}
	...
	if (ctfc) multi_send_flag_drop(Player_num);		// jinx 01-04-13 ctfc
...
multi_process_data(char *buf, int len)
{
	...
	switch(type)
	{
		...
		case MULTI_KILL_CLIENT:
			multi_do_kill(buf); break;
		case MULTI_DO_FLAG_SPAWNS:
			do_flag_spawns(buf); break;
		case MULTI_REQUEST_FLAG_SPAWNS:
			send_flag_spawns(buf); break;
		case MULTI_CAPTURE_BONUS:
			if (!Endlevel_sequence) multi_do_capture_bonus(buf); break;
		case MULTI_RETURN_BONUS:
			if (!Endlevel_sequence) multi_do_return_bonus(buf); break;
		case MULTI_AUTORETURN_RED:
			if (!Endlevel_sequence) multi_do_autoreturn_red(buf); break;
		case MULTI_AUTORETURN_BLUE:
			if (!Endlevel_sequence) multi_do_autoreturn_blue(buf); break;
		case MULTI_SEND_CAPTURE_FLAG:
			if (!Endlevel_sequence) multi_do_capture_flag(buf);	break;
		case MULTI_SEND_RETURN_FLAG:
			if (!Endlevel_sequence) multi_do_return_flag(buf);	break;
		case MULTI_DO_DENY:
			if (!Endlevel_sequence) multi_do_denied(buf); break;
		case MULTI_DO_UPDATE_STATS:
			if (!Endlevel_sequence) multi_do_update_stats(buf); break;
		case MULTI_DO_FLAG_DROP:
			if (!Endlevel_sequence) multi_do_flag_drop(buf); break;
		case DO_FLAG_DISAPPEAR:
			if (!Endlevel_sequence) multi_do_flag_disappearance(buf); break;
		case MULTI_DO_KILLSTREAK:
			if (!Endlevel_sequence) multi_do_killstreak(buf);	break;
		case MULTI_DO_LAST_KILLED:
			if (!Endlevel_sequence) multi_do_last_killed(buf);	break;
		case MULTI_DO_SPEC_STATUS:
			if (!Endlevel_sequence) multi_do_spec_status(buf);	break;		// jinx 01-28-13 spec
		case DO_I_AM_SPECTATING_YOU:
			if (!Endlevel_sequence) do_i_am_spectating_you(buf);	break;
		case DO_I_AM_NO_LONGER_SPECTATING_YOU:
			if (!Endlevel_sequence) do_i_am_no_longer_spectating_you(buf);	break;
		case REMAKE_PIGGY_INVISIBLE:
			if (!Endlevel_sequence) remake_piggy_invisible(buf);		break;
		case (MULTI_NEW_KILL_HOST|MULTI_NEW_KILL_CLIENT):
			if (!Endlevel_sequence) multi_new_do_kill(buf); break;
		default:
			Int3();
	}
}
...
void do_flag_spawns(char * buf)
{
/*
	int count = 1;
	
	red_flag_spawn = GET_INTEL_SHORT(buf + count); count += 4;
	blue_flag_spawn = GET_INTEL_SHORT(buf + count); count += 4;
*/
}

void send_flag_spawns()
{
/*
	int count = 0;

	multibuf[count] =  MULTI_DO_FLAG_SPAWNS;         count += 1;
	PUT_INTEL_SHORT(multibuf+count, red_flag_spawn );     count += 4;
	PUT_INTEL_SHORT(multibuf+count, blue_flag_spawn );     count += 4;
	multi_send_data(multibuf, count, 2);
	//multi_send_data_direct((ubyte *)multibuf, count, 0, 2);
	do_flag_spawns(multibuf);
*/
}
...
void multi_do_denied(char * buf)
{
	ignore_last_kill_text = 1;
}

void multi_send_denied(int killer, int killed)
{
	multibuf[0]=MULTI_DO_DENY;
	multi_send_data(multibuf, 1, 2);
	multi_do_denied(multibuf);
}

void multi_send_update_stats(int pnum, int type)		// jinx 01-03-13 misc
{
	int x = 0;
	multibuf[0]=MULTI_DO_UPDATE_STATS;
	multibuf[1]=pnum;
	multibuf[2]=type;
	switch (type)
	{
		case CAPTURE:
			x=Players[pnum].flag_captures_total; break;
		case RETURN:
			x=Players[pnum].flag_returns_total; break;
		case DENIAL:
			x=Players[pnum].flag_denials_total; break;
	}
	multibuf[3] = x;
	
	multi_send_data(multibuf, 4, 2);
	multi_do_update_stats(multibuf);
}

void multi_do_update_stats(char * buf)		// jinx 01-03-13 misc
{
	int pnum = buf[1];
	int type = buf[2];
	int x = buf[3];
	...
	{
		case CAPTURE:
			Players[pnum].flag_captures_total = x; break;
		case RETURN:
			Players[pnum].flag_returns_total = x; break;
		case DENIAL:
			Players[pnum].flag_denials_total = x; break;
	}
}
...
// ************************************
// jinx 01-25-12 instagib block start
// ************************************

void multi_send_killstreak (int killstreak)
{
	/*multibuf[0]=MULTI_DO_KILLSTREAK;
	multibuf[1]=killstreak;
	multibuf[2]=Player_num;
	multi_send_data(multibuf, 3, 2);
	multi_do_killstreak(multibuf);*/
}

void multi_do_killstreak (char * buf)
{
	int kill_streak = buf[1];
	int pnum = buf[2];
	HUD_init_message(HM_MULTI, "%s has a killstreak of %d!", Players[pnum].callsign, kill_streak);
	
}

void multi_send_last_killed (int last_killed)
{
	/*multibuf[0]=MULTI_DO_LAST_KILLED;
	multibuf[1]=last_killed;
	multibuf[2]=Player_num;
	multi_send_data(multibuf, 3, 2);
	multi_do_last_killed(multibuf);*/
}

void multi_do_last_killed (char * buf)
{
	int lastkilled = buf[1];
	int pnum = buf[2];
	
	int secs = (lastkilled) % 60;
	int mins = (lastkilled) / 60;

	HUD_init_message(HM_MULTI, "%s survived %d:%02d without dying!", Players[pnum].callsign, mins, secs);
}

// ************************************
// jinx 01-25-12 instagib block end
// ************************************
...
void multi_make_player_spec()
{
	...
	if ((obj->type != OBJ_PLAYER && (obj->type != OBJ_SPECTATOR))) return;
	
	in_free = 1;
	
	if (!(Players[Player_num].flags & PLAYER_FLAGS_SPECTATING) && (spec_trigger == 0))
	{
		...
		obj->movement_type	= MT_PHYSICS;
		multi_reset_player_object(obj);
		Players[Player_num].is_spectating = 1;
		Players[Player_num].flags |= PLAYER_FLAGS_SPECTATING;
		multi_send_position(Players[Player_num].objnum);
		multi_powcap_cap_objects();
		drop_player_eggs(ConsoleObject);
		multi_send_player_explode(MULTI_PLAYER_DROP);
	...
	if ((Players[Player_num].flags & PLAYER_FLAGS_SPECTATING) && (spec_trigger == 0))
	{
		...
		obj->movement_type	= MT_PHYSICS;
		multi_reset_player_object(obj);
		Players[Player_num].is_spectating = 0;
		Players[Player_num].flags &= ~PLAYER_FLAGS_SPECTATING;
		spec_trigger = 1;
	...
	if (Players[Player_num].flags & PLAYER_FLAGS_SPECTATING) type = 1;
	if (!(Players[Player_num].flags & PLAYER_FLAGS_SPECTATING)) type = 0;
...
void do_i_am_spectating_you(char * buf)
{
	...
	int spec_num = buf[1];
	Players[spec_num].flags |= PLAYER_FLAGS_SPECTATING_ME;
...
void maybe_send_my_death_to_spectators()
{
	/*for (int i = 0; i < N_players; i++)
		if (Players[i].flags & PLAYER_FLAGS_SPECTATING_ME)
			send_my_death_to_spectators(i);*/
}

void send_my_death_to_spectators(int pnum)
{
	multibuf[0]=REMAKE_PIGGY_INVISIBLE;
	multibuf[1]=Player_num;
	multibuf[2]=pnum;
	multi_send_data(multibuf, 3, 1);
}
...
void make_nonpiggies_visible(int piggy_num)
{
	for (int i = 0; i < N_players; i++)
	{
		if (!(Players[i].flags & PLAYER_FLAGS_SPECTATING) && (i != piggy_num))
		{
			object * obj;
...
void switch_between_piggy_and_free()
{
	...
	{
		in_free = 1;
		piggy_and_free_switch = 1;
		make_nonpiggies_visible(-1);
		send_i_am_no_longer_spectating_you(piggy_num);
	}
	if ((in_free == 1) && (piggy_and_free_switch == 0))
	{
		in_free = 0;
...
//	int i;
	...
	piggy_num++;
	if (piggy_num >= N_players) piggy_num = 0;
	/*for (i = piggy_num; i != last_piggy_num; i++)
	{
		if (!(Players[i].flags & PLAYER_FLAGS_SPECTATING)) break;
		if (piggy_num == Player_num) i++;
		if (piggy_num >= N_players) piggy_num = 0;
	}*/
	
	//if (!(Players[i].flags & PLAYER_FLAGS_SPECTATING))
	//{
	//	piggy_num = i;
		HUD_init_message(HM_MULTI, "now spectating %s", Players[piggy_num].callsign);
		make_nonpiggies_visible(piggy_num);
	...
	//else
	//{
	//	HUD_init_message(HM_MULTI, "Nobody to spectate, returning to free fly mode!");
...
void remake_piggy_invisible(char * buf)
{
	int me = buf[2];
	if (me != Player_num) return;
	int you = buf[1];
	
	object * obj;
	obj = &Objects[Players[you].objnum];
...
{
	if (!(Players[Player_num].flags & PLAYER_FLAGS_SPECTATING)) return;
...
// ************************************
// jinx 02-04-13 descent 1 block start
// ************************************
...
int D4Equipped()
{
	...
	if (checked==-1)
	{
		if (PHYSFSX_exists("descent4.ham",1))
			checked=1;
		else
			checked=0;
	}
...
{
	/*
	if (Netgame.Descent4 == 1)
		HUD_init_message(HM_MULTI, "Now in D1 mode");
	if (Netgame.Descent4 == 0)
		HUD_init_message(HM_MULTI, "Now in D2 mode");
		*/
}
...
{
/*	new_ham = hamfile;
		if (PHYSFSX_exists(&hamfile,1))
		{
			HUD_init_message(HM_CONSOLE, "loading ham: %s", &hamfile);
		}
		else
		{
			HUD_init_message(HM_CONSOLE, "error: %s not found", &new_ham);
		}
*/
}
...
void reload_hamfile_first_time()
{
	if (d4_first_time == 0)
	{
		//load_hamfile(DEFAULT_D4HAMFILE_REGISTERED);
		read_hamfile();
		d4_first_time = 1;
	}
}
...
// ************************************
// jinx 02-04-13 descent 1 block end
// ************************************

// ************************************
// jinx 02-04-13 misc block start
// ************************************
/*
void multi_send_make_new_host(int pnum)
{
	multibuf[0]=DO_NEW_HOST;
	multibuf[1]=Player_num;
	multibuf[2]=pnum;
	multi_send_data(multibuf, 3, 2);
	multi_do_make_new_host(multibuf);
}

void multi_do_make_new_host(char * buf)
{
	int middle;
	you = buf[1];
	me = buf[2];
...
void set_killer_weapon(int wid, int wtype)
{
	...
	if (!new_kill_text) return;
	if (Players[Player_num].flags & (OF_SHOULD_BE_DEAD|OF_EXPLODING)) return;
	//HUD_init_message(HM_CONSOLE, "1: pre-switch: hit by id: %d type: %d", wid, wtype);
	
	Players[Player_num].kwid = wid;
	Players[Player_num].kwtype = wtype;
		
	int kwid;
	int kwtype;
		
		if (wid== 	FLARE_ID) 									{ kwtype = 5; kwid = -1; 	}
		if (wid== 	LASER_ID_L1) 								{ kwtype = 5; kwid = 0; 	}
		if (wid== 	LASER_ID_L2) 								{ kwtype = 5; kwid = 0; 	}
		if (wid== 	LASER_ID_L3) 								{ kwtype = 5; kwid = 0; 	}
		if (wid== 	LASER_ID_L4) 								{ kwtype = 5; kwid = 0; 	}
		if (wid== 	CONCUSSION_ID) 						{ kwtype = 5; kwid = 9; 	}
		if (wid== 	VULCAN_ID) 								{ kwtype = 5; kwid =  1; 	}
		if (wid== 	SPREADFIRE_ID) 						{ kwtype = 5; kwid =  3; 	}
		if (wid== 	PLASMA_ID) 								{ kwtype = 5; kwid =  5; 	}
		if (wid== 	FUSION_ID) 									{ kwtype = 5; kwid =  7; 	}
		if (wid== 	HOMING_ID) 								{ kwtype = 5; kwid =  11; }	
		if (wid== 	PROXIMITY_ID) 							{ kwtype = 5; kwid =  13; }	
		if (wid== 	SMART_ID) 									{ kwtype = 5; kwid = 15 ; }	
		if (wid== 	MEGA_ID) 									{ kwtype = 5; kwid =  17; }	
		if (wid== 	PLAYER_SMART_HOMING_ID) 	{ kwtype = 5; kwid =  15; }	
		if (wid== 	EARTHSHAKER_MEGA_ID) 			{ kwtype = 5; kwid =  18; }	
		if (wid== 	SUPER_LASER_ID) 						{ kwtype = 5; kwid = 19;	}
		if (wid== 	LASER_ID_L6) 								{ kwtype = 5; kwid =  19;}
		if (wid== 	GAUSS_ID) 									{ kwtype = 5; kwid =  2; 	}
		if (wid== 	HELIX_ID) 									{ kwtype = 5; kwid =  4; 	}
		if (wid== 	PHOENIX_ID) 								{ kwtype = 5; kwid =  6; 	}
		if (wid== 	OMEGA_ID) 									{ kwtype = 5; kwid =  8; 	}
		if (wid== 	FLASH_ID) 									{ kwtype = 5; kwid =  10; }	
		if (wid== 	GUIDEDMISS_ID) 							{ kwtype = 5; kwid = 12 ; }	
		if (wid== 	SUPERPROX_ID) 							{ kwtype = 5; kwid =  14; }	
		if (wid== 	MERCURY_ID) 								{ kwtype = 5; kwid =  16; }	
		if (wid== 	EARTHSHAKER_ID) 						{ kwtype = 5; kwid =  18; }	
		if (wid== 	SMART_MINE_HOMING_ID) 			{ kwtype = 5; kwid =  14; }

	kwtype = wtype;
		
	//HUD_init_message(HM_CONSOLE, "2: post-switch pre-index hit by id: %d type: %d", kwid, kwtype);
	Players[Player_num].kwid = kwid;
	Players[Player_num].kwtype = kwtype;
	//HUD_init_message(HM_CONSOLE, "3: post-switch post-index by id: %d type: %d", Players[Player_num].kwid, Players[Player_num].kwtype);
	//multi_new_compute_kill(Player_num, Player_num, Players[Player_num].kwid, Players[Player_num].kwtype);
}

void show_killer_weapon_type(int killer, int killed, int kwid, int kwtype)
{
	//HUD_init_message(HM_MULTI, "killed by id: %d type: %d", Players[Player_num].kwid, Players[Player_num].kwtype);
		
	if (killer == killed)
		kwtype = 244;
		
	if (kwtype == 244)
		HUD_init_message(HM_MULTI, "%s fumbles for the pilot's manual", Players[killed].callsign);
	else if (kwtype == 245)
		HUD_init_message(HM_MULTI, "%s was killed by the reactor", Players[killed].callsign);
	else if (kwtype == 246)
		HUD_init_message(HM_MULTI, "%s had a nuclear meltdown", Players[killed].callsign);
	else if (kwtype == 247)
		HUD_init_message(HM_MULTI, "%s was killed by a force field", Players[killed].callsign);
	else if (kwtype == 248)
		HUD_init_message(HM_MULTI, "%s was telefragged by a robot", Players[killed].callsign);
	else if (kwtype == 249)
		HUD_init_message(HM_MULTI, "%s was killed by the environment", Players[killed].callsign);
	else if (kwtype == 250)
		HUD_init_message(HM_MULTI, "%s was hugged to death", Players[killed].callsign);
	else if (kwtype == 5)
		do_normal_player_killed_player_with_message(killer, killed, kwid);
	else 
		HUD_init_message(HM_MULTI, "%s was killed by an unindexed object id: %d type: %d", Players[killed].callsign, Players[killed].kwid, Players[killed].kwtype);
		
//	for (int i = 0; i < MAX_PRIMARY_WEAPONS + 15; i++)
//		HUD_init_message(HM_MULTI, "name: %s id: %d type: %d", PRIMARY_WEAPON_NAMES(i),i, Players[Player_num].kwtype);
	
	
}
void do_normal_player_killed_player_with_message(int killer, int killed, int weapon)
{
	int kwid = weapon;
	int killer_pnum = killer;
	int killed_pnum = killed;
	...
	if (kwid == -1) HUD_init_message(HM_MULTI, "%s killed %s with flares!" , killer_name, killed_name);
	if (kwid == 0) HUD_init_message(HM_MULTI, "%s killed %s with lasers", killer_name, killed_name);		
	if (kwid == 19) HUD_init_message(HM_MULTI, "%s killed %s with super lasers", killer_name, killed_name);		
	if (kwid == 1) HUD_init_message(HM_MULTI, "%s killed %s with vulcan", killer_name, killed_name);		
	if (kwid == 2) HUD_init_message(HM_MULTI, "%s killed %s with gauss", killer_name, killed_name);		
	if (kwid == 3) HUD_init_message(HM_MULTI, "%s killed %s with spreadfire", killer_name, killed_name);		
	if (kwid == 4) HUD_init_message(HM_MULTI, "%s killed %s with helix", killer_name, killed_name);		
	if (kwid == 5) HUD_init_message(HM_MULTI, "%s killed %s with plasma", killer_name, killed_name);		
	if (kwid == 6) HUD_init_message(HM_MULTI, "%s killed %s with phoenix", killer_name, killed_name);		
	if (kwid == 7) HUD_init_message(HM_MULTI, "%s killed %s with fusion", killer_name, killed_name);		
	if (kwid == 8) HUD_init_message(HM_MULTI, "%s killed %s with omega", killer_name, killed_name);		
	if (kwid == 9) HUD_init_message(HM_MULTI, "%s killed %s with a concussion missile", killer_name, killed_name);		
	if (kwid == 10) HUD_init_message(HM_MULTI, "%s killed %s with a flash missile", killer_name, killed_name);		
	if (kwid == 11) HUD_init_message(HM_MULTI, "%s killed %s with a homer missile", killer_name, killed_name);		
	if (kwid == 12) HUD_init_message(HM_MULTI, "%s killed %s with a guided missile", killer_name, killed_name);		
	if (kwid == 13) HUD_init_message(HM_MULTI, "%s killed %s with a proximity mine", killer_name, killed_name);		
	if (kwid == 14) HUD_init_message(HM_MULTI, "%s killed %s with a smart mine", killer_name, killed_name);		
	if (kwid == 15) HUD_init_message(HM_MULTI, "%s killed %s with a smart missile", killer_name, killed_name);		
	if (kwid == 16) HUD_init_message(HM_MULTI, "%s killed %s with a mercury missile", killer_name, killed_name);		
	if (kwid == 17) HUD_init_message(HM_MULTI, "%s killed %s with a mega missile", killer_name, killed_name);		
	if (kwid == 18) HUD_init_message(HM_MULTI, "%s killed %s with an earthshaker missile", killer_name, killed_name);		
}
/*	"Laser Cannon",		0
	"Vulcan Cannon",		1
	"Gauss Cannon",		2
	...
	"Mercury Missile",		16
	"Mega Missile",			17
	"Earthshaker Missile",18
	"Super Lasers"			19
*/


// ************************************
// jinx 02-04-13 misc block end
// ************************************
...
void multi_new_compute_kill(int killer, int killed, int kwid, int kwtype)
{
	int killed_pnum, killed_type;
	int killer_pnum, killer_type,killer_id;
	int killer_weapon, killer_weapon_type;
	int TheGoal;
	...
	if (killer_type == OBJ_CNTRLCEN)
	{
		...
		show_killer_weapon_type(0, killed_pnum, kwid, -7);
		return;
	}

	else if ((killer_type != OBJ_PLAYER) && (killer_type != OBJ_GHOST))
	{
		show_killer_weapon_type(0, killed_pnum, kwid, -2);
		Players[killed_pnum].net_killed_total++;
		return;
	}
	...
	if (killer_pnum == killed_pnum)
	{
		...
		show_killer_weapon_type(killer_pnum, killed_pnum, killer_weapon, killer_weapon_type);
	...
	else
	{
		...
		show_killer_weapon_type(killer_pnum, killed_pnum, kwid, kwtype);
	}
	...
	HUD_init_message(HM_MULTI, "new_compute() killer_pnum: %d killed_pnum: %d kwid: %d kwtype: %d", killer_pnum, killed_pnum, kwid, kwtype);		
}
...
void multi_new_do_kill(char *buf)
{
	...
	int killed = (int)(buf[count]);
	int kwid = (int)(buf[7]);
	int kwtype = (int)(buf[8]);
	
	/*if (multi_i_am_master() && type != MULTI_NEW_KILL_CLIENT)
		return;
	if (!multi_i_am_master() && type != MULTI_NEW_KILL_HOST)
		return;*/
	...
	{
		memcpy(multibuf, buf, 5);
		multibuf[0] = MULTI_NEW_KILL_HOST;
		multibuf[5] = Netgame.team_vector;
		multibuf[6] = Bounty_target;
		multibuf[7] = kwid;
		multibuf[8] = kwtype;
	...
	HUD_init_message(HM_MULTI, "new_do() killer_pnum: %d killed_pnum: %d kwid: %d kwtype: %d", killer, killed, kwid, kwtype);		
	//multi_new_compute_kill(killer, killed, kwid, kwtype);		// jinx 02-05-13 weapon types
...
void multi_new_send_kill(int objnum)
{
	// I died, tell the world.

	int killer_objnum;
	int count = 0;
	int kwid = Players[Player_num].kwid;
	int kwtype = Players[Player_num].kwtype;
	...
	if (multi_i_am_master())
		multibuf[count] = (char)MULTI_NEW_KILL_HOST;
	else
		multibuf[count] = (char)MULTI_NEW_KILL_CLIENT;
	...
	multibuf[count] = kwid;	count += 1;
	multibuf[count] = kwtype;	count += 1;
	
	
	if (multi_i_am_master())
	{
		HUD_init_message(HM_MULTI, "new_send() killer_pnum: %d killed_pnum: %d kwid: %d kwtype: %d", killer_objnum, objnum, Players[Player_num].kwid, Players[Player_num].kwtype);
		//multi_new_compute_kill(killer_objnum, objnum, Players[Player_num].kwid, Players[Player_num].kwtype);
		multi_send_data(multibuf, count, 2);
		multi_new_do_kill(multibuf);
	}
	else
		multi_send_data_direct((ubyte*)multibuf, count, multi_who_is_master(), 2); // I am just a client so I'll only send my kill but not compute it, yet. I'll get response from host so I can compute it correctly
