...
void do_weapon_n_item_stuff()
{
	...
	if (allowed_to_fire_missile() && Controls.fire_secondary_state && !instagib)
	...
	while (Controls.drop_bomb_count > 0 && !instagib)
	{
		do_missile_firing(1);
...
int HandleDeathInput(d_event *event)
{
	...
	if ((Game_mode & GM_CAPTURE) && (player_spawn_timer+(Netgame.RespawnTime*5)) > f2i(Players[Player_num].time_level) && Player_exploded)
	{
		...
		return 0; 	// jinx 12-23-12 ctfc
	}
	if ((Game_mode & GM_CAPTURE) && (force_respawn) && (player_spawn_timer+(Netgame.RespawnTime*5)) <= f2i(Players[Player_num].time_level))
	{
		...
		case CV_COOP:
			...
			if ((Game_mode & GM_MULTI_COOP) || (Game_mode & GM_TEAM)) {
				...
				while (1) 
				{
					...
					if (Coop_view_player[w]==Player_num)	// jinx 01-04-13 ctfc: camera for following flag carrier
					{
...
// weapon selection handled in kconfig_read_controls, d1x-style
			...
			case KEY_0 + KEY_ALTED + KEY_SHIFTED:	// jinx 01-25-13 spec
				multi_make_player_spec(Player_num);
				game_flush_inputs();
				break;
#endif
