...
void multi_compute_kill(int killer, int killed)
{
	...
	if (killer_pnum == killed_pnum)
	{
		...
		if (killer_pnum == Player_num)
		{
			if (ignore_last_kill_text) 
			{
				HUD_init_message(HM_MULTI, "you denied yourself!");
				ignore_last_kill_text = 0;
			}
			else
		...
		else
		{
			if (ignore_last_kill_text) 
			{
				HUD_init_message(HM_MULTI, "%s denied himself!", killed_name);
				ignore_last_kill_text = 0;
			}
			else
	...
	else
	{
		...
		if (killer_pnum == Player_num) {
			if (ignore_last_kill_text) 
			{
				HUD_init_message(HM_MULTI, "You denied %s!", killed_name);
				ignore_last_kill_text = 0;
			}
			else
		...
		else if (killed_pnum == Player_num)
		{
			if (ignore_last_kill_text) 
			{
				HUD_init_message(HM_MULTI, "%s denied you!", killer_name);
				ignore_last_kill_text = 0;
			}
			else
		...
		else
		{
			if (ignore_last_kill_text) 
			{
				HUD_init_message(HM_MULTI, "%s denied %s!", killer_name, killed_name);
				ignore_last_kill_text = 0;
			}
			else
...
void multi_make_player_spec(int pnum)
{
	...
	if (!spec) return;
	if (Players[pnum].flags & PLAYER_FLAGS_FLAG)
	{
	...
	/*
	if ((obj->type == OBJ_PLAYER) && (spec_trigger == 0))
	{
	...
	if ((obj->type == OBJ_SPECTATOR) && (spec_trigger == 0))
	{
		obj->type = OBJ_PLAYER;
	...
	*/
	...
	multi_send_spec_status(pnum, obj->type);
...
void multi_send_spec_status (int pnum, int type)
{
	...
	/*if (Players[Player_num].is_spectating == 0) 
	{
...
void multi_do_spec_status (char * buf)
{
	...
	if ((Players[pnum].is_spectating != 1) && (spec_trigger == 0))
	{
		obj->type = OBJ_SPECTATOR;
		obj->render_type = RT_NONE;
		obj->control_type	= CT_FLYING;
		...
		if (pnum == Player_num) reset_stats_spec();
	}
	if ((Players[pnum].is_spectating) && (spec_trigger == 0))
	{
		obj->type = OBJ_PLAYER;
		obj->render_type = RT_POLYOBJ;
		obj->control_type	= CT_FLYING;
...
extern void net_udp_send_door_updates(int pnum);

void multi_request_door_updates(int pnum)
{
	/*if (!multi_i_am_master()) return;
	net_udp_send_door_updates(pnum);*/
}
...
void multi_send_spec_status_to_players()
{
	if (!multi_i_am_master()) return;
	if (multi_i_am_master())
	{
...
void multi_do_spec_status_of_players()
{
...
// ************************************
// jinx 01-25-12 spectator block end
// ************************************
...
void load_hamfile(char hamfile)
{
	new_ham = hamfile;
		...
		{
			HUD_init_message(HM_CONSOLE, "loading ham %s", &new_ham);
		}
		else
		{
