...
void hud_show_weapons(void)
{
	...
	else
	{
		...
		if (!instagib) sprintf(weapon_str, "%s %d",weapon_name,Players[Player_num].secondary_ammo[Secondary_weapon]);
		if (instagib) 
		{
...
void draw_weapon_info(int weapon_type,int weapon_num,int laser_level)
{
	...
	if (weapon_type == 0) {
		...
		if (instagib)
		{
			SECONDARY_WEAPON_NAMES_SHORT(weapon_num) = "blahblahblahblahblah";
			if (PlayerCfg.CockpitMode[1] == CM_STATUS_BAR)
				draw_weapon_info_sub(info_index,
					&gauge_boxes[SB_SECONDARY_BOX],
						0,0,
