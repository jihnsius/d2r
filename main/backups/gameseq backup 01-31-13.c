...
gameseq_init_network_players()
{
	...
	for (i=0;i<=Highest_object_index;i++) {
		...
		if (( Objects[i].type==OBJ_PLAYER )	|| (Objects[i].type == OBJ_GHOST) || (Objects[i].type == OBJ_COOP) || (Objects[i].type == OBJ_SPECTATOR))
		{
...
void init_player_stats_game(ubyte pnum)
{
	...
	Players[pnum].flag_denials_total = 0;
...
void init_player_stats_level(int secret_flag)
{
	...
	Players[Player_num].is_spectating = 0;	// jinx 01-25-13 spec
}
...
/*	int r = 0;
	...
	if (Players[Player_num].is_spectating == 1)  HUD_init_message(HM_MULTI, "spectating");
}
...
void flag_classic_main()		// jinx 01-06-13 ctfc: main loop for ctfc (timers and stuff)
{
	...
	if (!(Players[Player_num].flags & PLAYER_FLAGS_FLAG))	// if a player doesn't have a flag, turn everything off
	{
		can_be_denied = 0;
		deny_timer = current_time;
		deny_countdown = DENY_TIME + 1;
	}
	if (Players[Player_num].flags & PLAYER_FLAGS_FLAG) 
	{
		if (deny_timer != current_time)
		{
			deny_timer = current_time;
			deny_countdown += 1;
		}
		if (deny_countdown <= DENY_TIME && Players[Player_num].flags & PLAYER_FLAGS_FLAG)
			can_be_denied = 1;
		if (deny_countdown > DENY_TIME)
			can_be_denied = 0;
	}	
...
void spectator_main()
{
	if (Players[Player_num].is_spectating)
	{
		for (int i = 0; i <= Num_walls; i++)
		{
			...
			{
				Walls[i].spect = 1;
				if (Walls[i].state == WALL_DOOR_OPEN)
					Walls[i].cloak_value = 9;
				if (Walls[i].state == WALL_DOOR_CLOSED)
					Walls[i].cloak_value = 6;
			}
		}
		
	}
	if (Players[Player_num].is_spectating == 2)
	{
		Players[Player_num].is_spectating = 0;
		/*if (!multi_i_am_master())
		{
			//multi_request_door_updates(Player_num);
			for (int i = 0; i <= Num_walls; i++)
				if (Walls[i].spect == 1) 
					Walls[i].spect = 0;
		}
	...
	{
		object * obj;
		obj = &Objects[Players[n].objnum];
		if (Players[n].flags & PLAYER_FLAGS_SPECTATING)
			Players[n].is_spectating = 1;
		if (!(Players[n].flags & PLAYER_FLAGS_SPECTATING))
			Players[n].is_spectating = 0;
		if (Players[n].is_spectating == 1)
		{
			obj->type = OBJ_SPECTATOR;
			obj->render_type = RT_NONE;
			obj->control_type	= CT_FLYING;
			obj->movement_type	= MT_PHYSICS;
			Players[n].flags |= PLAYER_FLAGS_SPECTATING;
		}
		if (Players[n].is_spectating == 0)
		{
			obj->type = OBJ_PLAYER;
...
void InitPlayerPosition(int random_flag)
{
	...
	if (Game_mode & GM_CAPTURE)		// jinx 12-16-12 ctfc players spawn on their corresponding sides (red is odd, blue is even)
	{
		first_spawns = 1;
		//NewPlayer = d_rand() % NumNetPlayerPositions;
