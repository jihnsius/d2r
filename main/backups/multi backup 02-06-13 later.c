...
multi_leave_game(void)
{
	...
	if (Game_mode & GM_NETWORK)
	{
		Players[Player_num].flags &= ~PLAYER_FLAGS_SPECTATING;		// jinx 01-26-13 spec
		Net_create_loc = 0;
...
void multi_send_message_end()
{
	...
	else if (!d_strnicmp (Network_message,"/kill",5) && (Game_mode & GM_NETWORK) && !Control_center_destroyed)
	{
		...
		console_kill = 1;
		set_killer_weapon(0, 244);		// jinx 02-06-13 weapon text
...
void multi_disconnect_player(int pnum)
{
	...
	Players[pnum].flags &= ~PLAYER_FLAGS_SPECTATING;		// jinx 02-06-13 spec
...
void multi_reset_player_object(object *objp)
{
	...
	Assert((objp->type == OBJ_PLAYER) || (objp->type == OBJ_GHOST) || (objp->type == OBJ_SPECTATOR) || (objp->type == OBJ_SPECTATEE));	// jinx 01-26-13 spec
	...
	if ((objp->type == OBJ_GHOST) || (objp->type == OBJ_SPECTATOR) || (objp->type == OBJ_SPECTATEE))		// jinx 01-25-13 spec
...
void multi_object_to_object_rw(object *obj, object_rw *obj_rw)
{
	...
	switch (obj_rw->render_type)
	{
		...
		{
			int i;
			if (obj->render_type == RT_NONE && (obj->type != OBJ_GHOST && obj->type != OBJ_SPECTATOR && obj->type != OBJ_SPECTATEE)) // HACK: when a player is dead or not connected yet, clients still expect to get polyobj data - even if render_type == RT_NONE at this time.
...
void multi_make_player_spec()
{
	...
	if ((obj->type != OBJ_PLAYER) && (obj->type != OBJ_SPECTATOR) && (obj->type != OBJ_SPECTATEE)) return;
...
void make_piggy_invisible(int piggy_num)
{
	...
	obj->type = OBJ_SPECTATEE;
	obj->render_type = RT_NONE;
}

void make_last_piggy_visible(int piggy_num)
{
	if (Players[piggy_num].flags & PLAYER_FLAGS_SPECTATING) return;
	object * obj;
	obj = &Objects[Players[piggy_num].objnum];
	if (obj->type == OBJ_GHOST || obj->type == OBJ_SPECTATOR) return;
...
void switch_between_piggies()
{
	...
	if ((piggy_num != last_piggy_num) && !testing)
	{
...
void is_it_working()
{
	HUD_init_message(HM_CONSOLE, "it is working");
}
...
// ************************************
// jinx 02-06-13 new kill text start
// ************************************
...
void multi_new_do_kill(char *buf)
{
	...
	if (multi_i_am_master() && type != MULTI_NEW_KILL_CLIENT && !testing)
		return;
	if (!multi_i_am_master() && type != MULTI_NEW_KILL_HOST && !testing)
		return;
...
// ************************************
// jinx 02-06-13 new kill text end
// ************************************