...
void show_netplayerinfo()
{
	...
	else if (Game_mode & GM_CAPTURE)	// jinx 01-03-13
	{
		gr_printf(x+FSPACX(8)*5,y,"kills");
		gr_printf(x+FSPACX(8)*8,y,"deaths");
		gr_printf(x+FSPACX(8)*13,y,"pt");
		gr_printf(x+FSPACX(8)*15,y,"cp");
		gr_printf(x+FSPACX(8)*17,y,"rt");
		gr_printf(x+FSPACX(8)*19,y,"dn");
		gr_printf(x+FSPACX(8)*21,y,"tk");
		gr_printf(x+FSPACX(8)*23,y,"ping");
		gr_printf(x+FSPACX(8)*26,y,"eff");
	}
	else
	{
	...
	for (i=0; i<MAX_PLAYERS; i++)
	{
		...
		else if (Game_mode & GM_CAPTURE && ctfc)		// jinx 01-04-13 ctfc
		{
			...
			gr_printf(x+FSPACX(8)*19,y,"%-6d",Players[i].flag_denials_total);
			gr_printf(x+FSPACX(8)*21,y,"%-6d",Players[i].net_teamkills_total);
			gr_printf(x+FSPACX(8)*23,y,"%-6d",Netgame.players[i].ping);
			gr_printf(x+FSPACX(8)*26,y,"%d/%d",kill_matrix[Player_num][i],kill_matrix[i][Player_num]);
		}
		else
		{
		...
		switch (PlayerCfg.Cockpit3DView[w]) {
			...
			case CV_COOP: {
	         ...
	         RenderingType=255; // don't handle coop stuff			
				...
				{
					if ((player == Player_num) && (Game_mode & GM_CAPTURE) && ctfc)		// jinx 01-05-13 ctfc: show whoever has the flag's screen, if no one has the flag, show your screen
					{
						for (int i = 0;i<=N_players;i++)
						{
							if ((Players[i].flags & PLAYER_FLAGS_FLAG) && (get_team(i)==get_team(Player_num)))
							{
								char name[8];
								sprintf(name,"flag: %s",Players[i].callsign);
								do_cockpit_window_view(w,&Objects[Players[i].objnum],0,WBU_COOP,name);
								break;
							}
							if (i==N_players && !(Players[i].flags & PLAYER_FLAGS_FLAG))
							{
								do_cockpit_window_view(w,&Objects[Players[Player_num].objnum],0,WBU_COOP,"self");
							}
						}
					}
					else
...
extern void show_proxy_box();	// jinx 01-10-13 ctfc
extern void show_flag_indicator();	// jinx 01-10-13 ctfc
...
void game_render_frame_mono(int flip)
{
	...
	if (Guided_missile[Player_num] && Guided_missile[Player_num]->type==OBJ_WEAPON && Guided_missile[Player_num]->id==GUIDEDMISS_ID && Guided_missile[Player_num]->signature==Guided_missile_sig[Player_num] && PlayerCfg.GuidedInBigWindow) {
		...
		if (Game_mode & GM_TEAM)
			show_proxy_box();		// jinx 01-10-13 ctfc
