...
void apply_force_damage(object *obj,fix force,object *other_obj)
{
	...
	switch (obj->type) {
		...
		case OBJ_PLAYER:
			...
			set_killer_weapon(other_obj->id, other_obj->type);		// jinx 02-05-13 weapon text
		...
		if (!(Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE))
			if ( Players[Player_num].shields > f1_0*10 || ForceFieldHit)
			{
				if (TmapInfo[tmap_num].flags & TMI_FORCE_FIELD)
					set_killer_weapon(0, 247);			// jinx 02-05-13 weapon types
				else
					set_killer_weapon(0, 249);			// jinx 02-05-13 weapon types
	...
	if (d > 0 || water) {
		...
		if (obj->id == Player_num) {
			...
			if (d > 0) {
				...
				if (!(Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE))
				{
					set_killer_weapon(0, 249);		// jinx 02-05-13 weapon text
...
void drop_player_eggs(object *playerobj)
{
	if ((playerobj->type == OBJ_PLAYER) || (playerobj->type == OBJ_GHOST) || (playerobj->type == OBJ_SPECTATOR)) 	// jinx 01-26-13 spec
	{
		...
		if (!(Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING))		// jinx 01-26-13 spec
			drop_missile_1_or_4(playerobj,CONCUSSION_INDEX);
		if ((Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING) && (CONCUSSION_INDEX > 4))	// jinx 01-26-13 spec
			drop_missile_1_or_4(playerobj,(CONCUSSION_INDEX-4));
...
void apply_damage_to_player(object *playerobj, object *killer, fix damage, ubyte possibly_friendly)
{
	...
	if ((Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE) && (!console_kill))
		return;
...
void collide_player_and_weapon( object * playerobj, object * weapon, vms_vector *collision_point )
{
	...
	if (weapon->id == 34)		// jinx 02-14-13 promod
	{
		if (weapon->mtype.phys_info.flags & PF_BOUNCED_ONCE)		
		{
			damage /= PHOENIX_DAMAGE_DIVISOR_1;
			HUD_init_message(HM_CONSOLE, "phoenix (bounce: 1) (damage: %d)", damage);
		}
		else if (!(weapon->mtype.phys_info.flags & PF_BOUNCE))
		{
			damage /= PHOENIX_DAMAGE_DIVISOR_2;
			HUD_init_message(HM_CONSOLE, "phoenix (bounce: 2) (damage: %d)", damage);
		}
		else
			HUD_init_message(HM_CONSOLE, "phoenix (bounce: 0) (damage: %d)", damage);
	}
...
//			damage /= 4;
		if (!(weapon->flags & OF_HARMLESS))
		{
			set_killer_weapon(weapon->id, weapon->type);		// jinx 02-05-13 weapon text
...
//	if (!(Robot_info[robot->id].energy_drain && Players[playerobj->id].energy))
	...
	set_killer_weapon(0, 250);		// jinx 02-05-13 weapon text
...
//	digi_play_sample( SOUND_PLAYER_GOT_HIT, F1_0 );
	...
	set_killer_weapon(0, 248);		// jinx 02-05-13 weapon text
