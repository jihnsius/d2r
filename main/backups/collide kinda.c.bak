...
void apply_force_damage(object *obj,fix force,object *other_obj)
{
	...
	if (obj->flags & (OF_EXPLODING|OF_SHOULD_BE_DEAD))	// jinx 01-26-13 spec
...
void collide_player_and_wall( object * playerobj, fix hitspeed, short hitseg, short hitwall, vms_vector * hitpt)
{
	...
	if (damage >= WALL_DAMAGE_THRESHOLD) {
		...
		if (volume > 0 && !ForceFieldHit) {  // uhhhgly hack
			...
			#ifdef NETWORK
			if ((Game_mode & GM_MULTI) && (Players[Player_num].flags & PLAYER_FLAGS_SPECTATING == 0))	// jinx 01-28-13 spec
...
void drop_player_eggs(object *playerobj)
{
	if ((playerobj->type == OBJ_PLAYER) || (playerobj->type == OBJ_GHOST) || (playerobj->type == OBJ_SPECTATOR)) 	// jinx 01-26-13 spec
	{
		...
		if ((Players[Player_num].flags & PLAYER_FLAGS_SPECTATING != 1))		// jinx 01-26-13 spec
			drop_missile_1_or_4(playerobj,CONCUSSION_INDEX);
		if ((Players[Player_num].flags & PLAYER_FLAGS_SPECTATING) && (CONCUSSION_INDEX > 4))	// jinx 01-26-13 spec
		...
		if ((Game_mode & GM_MULTI) && ((Players[Player_num].flags & PLAYER_FLAGS_SPECTATING != 1))) {		// jinx 01-26-13 spec
...
{
	if (Player_is_dead || Players[Player_num].flags & PLAYER_FLAGS_SPECTATING)	// jinx 01-25-13 spec
		return;
...
void collide_player_and_powerup( object * playerobj, object * powerup, vms_vector *collision_point ) {
	if (!Endlevel_sequence && !Player_is_dead && (playerobj->id == Player_num ) && (Players[Player_num].flags & PLAYER_FLAGS_SPECTATING != 1))		// jinx 01-25-13 spec
	{
		int powerup_used;
