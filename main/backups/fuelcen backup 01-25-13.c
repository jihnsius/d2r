...
void fuelcen_check_for_goal(segment *segp)
{
	...
	if (!(ctfc)) Netgame.FlagInHome = 0;
	
	if (seg2p->special == SEGMENT_IS_GOAL_BLUE)		// if player is in blue goal
	{
		if ((get_team(Player_num)==TEAM_BLUE) && (Players[Player_num].flags & PLAYER_FLAGS_FLAG)) // if blue player has flag
			{
				if (Netgame.FlagInHome == 1)	// if flag must be in goal to score
				{
					for (int b=0;b<=Highest_segment_index+1;b++)
					{
						if (Segment2s[b].special == SEGMENT_IS_GOAL_BLUE)	// if any segment is a blue goal
						{
							if (segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, b) || segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, blue_flag_spawn))		// and that segment contains a blue flag or the flag is in it's home
							{
								Players[Player_num].flags &=(~(PLAYER_FLAGS_FLAG));	
								multi_send_capture_flag(Player_num);
								return;		// break out of the loop so you only score once
							}
							if ((!(segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, b))))		// if flag isn't in it's goal and it's not in it's home
							{
								HUD_init_message(HM_MULTI,"You must return your flag to score!");		
							}
						}
					}
				}
				if (Netgame.FlagInHome == 0)		// if flag doesn't have to be in goal or home to score
				{
					Players[Player_num].flags &=(~(PLAYER_FLAGS_FLAG));
					multi_send_capture_flag(Player_num);
				}
	...
	{
		if ((get_team(Player_num)==TEAM_RED) && (Players[Player_num].flags & PLAYER_FLAGS_FLAG))	// if red player has flag
			{	
					if (Netgame.FlagInHome == 1)		// if flag must be in goal to score
					{
						for (int r=0;r<=Highest_segment_index+1;r++)
						{
							if (Segment2s[r].special == SEGMENT_IS_GOAL_RED)	// if any segment is a red goal
							{
								if (segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, r) || segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, red_flag_spawn))		// and that segment contains a blue flag or the flag is in it's home
								{			
									Players[Player_num].flags &=(~(PLAYER_FLAGS_FLAG));
									multi_send_capture_flag(Player_num);
									return;		// break out of the loop so you only score once
								}
								if ((!(segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, r))))		// if flag isn't in it's goal and isn't in it's home
								{
									HUD_init_message(HM_MULTI,"You must return your flag to score!");		
								}
							}
