...
int read_hamfile()
{
	...
	if ((!(Netgame.Descent1)) && !promod)
	{
		ham_fp = PHYSFSX_openReadBuffered(DEFAULT_HAMFILE_REGISTERED);
		HUD_init_message(HM_CONSOLE, "loading ham: %s", DEFAULT_HAMFILE_REGISTERED);
	}
	if (Netgame.Descent1)
	{
		ham_fp = PHYSFSX_openReadBuffered(DEFAULT_D1HAMFILE_REGISTERED);
		HUD_init_message(HM_CONSOLE, "loading ham: %s", DEFAULT_D1HAMFILE_REGISTERED);
	}
	if (promod)
	{
		ham_fp = PHYSFSX_openReadBuffered(DEFAULT_PROMOD_HAMFILE_REGISTERED);
		HUD_init_message(HM_CONSOLE, "loading ham: %s", DEFAULT_PROMOD_HAMFILE_REGISTERED);	
	}
	if (!ham_fp)
	{
