...
int create_weapon_object(int weapon_type,int segnum,vms_vector *position)
{
	...
	if (Netgame.Instagib && (weapon_type == bolt_type)) // if instagib and not flare and not reactor blob
	{	
		Weapon_info[weapon_type].persistent = 1;
		Weapon_info[weapon_type].fire_count = 1;
		Weapon_info[weapon_type].ammo_usage = 0;
		Weapon_info[weapon_type].energy_usage = 0;
		Weapon_info[weapon_type].fire_wait = instagun_firespeed;
		//if (gib_time >= current_time - gib_timer) { Weapon_info[weapon_type].fire_wait = instagun_firespeed / firespeed_bonus; }
		Weapon_info[weapon_type].strength[Difficulty_level] =  max_fix;	// max for fix (2^30)
		Weapon_info[weapon_type].speed[Difficulty_level] = max_fix; // max for fix (2^30)
		Weapon_info[weapon_type].mass = instagun_mass;
		Weapon_info[weapon_type].drag = 0;
		Weapon_info[weapon_type].thrust = 0;
		Weapon_info[weapon_type].light = max_fix;
		Weapon_info[weapon_type].flash = 0;
	}
...
#define OMEGA_BASE_TIME_OG ((Game_mode&GM_MULTI)?(F1_0/10):(F1_0/30)) // How many blobs per second!! No FPS-based blob creation anymore, no FPS-based damage anymore!
#define	MIN_OMEGA_BLOBS_OG		3				//	No matter how close the obstruction, at this many blobs created.
#define	MIN_OMEGA_DIST_OG			(F1_0*3)		//	At least this distance between blobs, unless doing so would violate MIN_OMEGA_BLOBS
#define	DESIRED_OMEGA_DIST_OG	(F1_0*5)		//	This is the desired distance between blobs.  For distances > MIN_OMEGA_BLOBS*DESIRED_OMEGA_DIST, but not very large, this will apply.
#define	MAX_OMEGA_BLOBS_OG		16				//	No matter how far away the obstruction, this is the maximum number of blobs.
#define	MAX_OMEGA_DIST_OG			(MAX_OMEGA_BLOBS_OG * DESIRED_OMEGA_DIST_OG)		//	Maximum extent of lightning blobs.
...
#define	OMEGA_MIN_TRACKABLE_DOT_OG			(15*F1_0/16)		//	Larger values mean narrower cone.  F1_0 means damn near impossible.  0 means 180 degree field of view.
#define	OMEGA_MAX_TRACKABLE_DIST_OG		MAX_OMEGA_DIST_OG	//	An object must be at least this close to be tracked.
...
#define	OMEGA_DAMAGE_SCALE_OG			32				//	Controls how much damage is done.  This gets multiplied by the damage specified in bitmaps.tbl in the $WEAPON line.
#define	OMEGA_ENERGY_CONSUMPTION_OG	16				//	Controls how much energy is consumed.  This gets multiplied by the energy parameter from bitmaps.tbl.
//	-------------------------------------------------------------------------------------------------------------------------------

#define	MIN_OMEGA_CHARGE_OG	(MAX_OMEGA_CHARGE/8)
#define	OMEGA_CHARGE_SCALE_OG	4			//	FrameTime / OMEGA_CHARGE_SCALE added to Omega_charge every frame.
#define	OMEGA_CHARGE_SCALE_OG	4
...
int OMEGA_BASE_TIME, MIN_OMEGA_BLOBS, MIN_OMEGA_DIST, DESIRED_OMEGA_DIST, MAX_OMEGA_BLOBS, MAX_OMEGA_DIST, OMEGA_MIN_TRACKABLE_DOT, OMEGA_MAX_TRACKABLE_DIST, OMEGA_DAMAGE_SCALE, OMEGA_ENERGY_CONSUMPTION;
int MIN_OMEGA_CHARGE, OMEGA_CHARGE_SCALE;
...
void promod_omega()		// jinx 02-14-13 promod
{
	if (!promod) 
	{
		OMEGA_BASE_TIME = OMEGA_BASE_TIME_OG;
		MIN_OMEGA_BLOBS = MIN_OMEGA_BLOBS_OG;
		MIN_OMEGA_DIST = MIN_OMEGA_DIST_OG;
		DESIRED_OMEGA_DIST = DESIRED_OMEGA_DIST_OG;
		MAX_OMEGA_BLOBS = MAX_OMEGA_BLOBS_OG;
		MAX_OMEGA_DIST = MAX_OMEGA_DIST_OG;
		OMEGA_MIN_TRACKABLE_DOT = OMEGA_MIN_TRACKABLE_DOT_OG;
		OMEGA_MAX_TRACKABLE_DIST = OMEGA_MAX_TRACKABLE_DIST_OG;
		OMEGA_DAMAGE_SCALE = OMEGA_DAMAGE_SCALE_OG; 
		OMEGA_ENERGY_CONSUMPTION = OMEGA_ENERGY_CONSUMPTION_OG;
		MIN_OMEGA_CHARGE = MIN_OMEGA_CHARGE_OG;
		OMEGA_CHARGE_SCALE = OMEGA_CHARGE_SCALE_OG;			//	FrameTime / OMEGA_CHARGE_SCALE added to Omega_charge every frame.
	}
	if (promod)
	{
		OMEGA_BASE_TIME = OMEGA_BASE_TIME_PROMOD;
		MIN_OMEGA_BLOBS = MIN_OMEGA_BLOBS_PROMOD;
		MIN_OMEGA_DIST = MIN_OMEGA_DIST_PROMOD;
		DESIRED_OMEGA_DIST = DESIRED_OMEGA_DIST_PROMOD;
		MAX_OMEGA_BLOBS = MAX_OMEGA_BLOBS_PROMOD;
		MAX_OMEGA_DIST = MAX_OMEGA_DIST_PROMOD;
		OMEGA_MIN_TRACKABLE_DOT = OMEGA_MIN_TRACKABLE_DOT_PROMOD;
		OMEGA_MAX_TRACKABLE_DIST = OMEGA_MAX_TRACKABLE_DIST_PROMOD;
		OMEGA_DAMAGE_SCALE = OMEGA_DAMAGE_SCALE_PROMOD; 
		OMEGA_ENERGY_CONSUMPTION = OMEGA_ENERGY_CONSUMPTION_PROMOD;
		MIN_OMEGA_CHARGE = MIN_OMEGA_CHARGE_PROMOD;
		OMEGA_CHARGE_SCALE = OMEGA_CHARGE_SCALE_PROMOD;
	}
}
...
{
	promod_omega();		// jinx 02-14-13 promod
...
int Laser_create_new( vms_vector * direction, vms_vector * position, int segnum, int parent, int weapon_type, int make_sound )
{
	...
	if (Weapon_info[obj->id].flash_sound > -1 )	{
		if (make_sound)	
		{
			if ((weapon_type == FLARE_ID) && promod)		// jinx 02-15-13 promod
				volume /= 3;
...
void Laser_player_fire_spread_delay(object *obj, int laser_type, int gun_num, fix spreadr, fix spreadu, fix delay_time, int make_sound, int harmless)
{
	...
	vms_matrix	m;
	int			objnum;
	int weapon_type = laser_type;
	
	
	if (Netgame.Instagib && (weapon_type != FLARE_ID) && (weapon_type != 6))
	{	
		Weapon_info[weapon_type].persistent = 1;
		Weapon_info[weapon_type].fire_count = 1;
		Weapon_info[weapon_type].ammo_usage = 0;
...
void homing_missile_turn_towards_velocity(object *objp, vms_vector *norm_vel)
{
	...
	if (Netgame.Descent1)
		vm_vec_scale(&new_fvec, FrameTime * D1_HOMING_MISSILE_SCALE);
	...
	if (Weapon_info[obj->id].homing_flag && !(obj->id==GUIDEDMISS_ID && obj->ctype.laser_info.parent_type==OBJ_PLAYER && obj==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id] && obj->signature==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id]->signature))
	{
		...
		if (obj->ctype.laser_info.creation_time + HOMING_MISSILE_STRAIGHT_TIME < GameTime64) {
			...
			if (track_goal != -1) {
				...
				if (promod)
					vm_vec_scale(&vector_to_object, F1_0/((float)(F1_0/promod_homer_turnspeed)/FrameTime));
...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	switch (weapon_num) {
		case LASER_INDEX: {
			...
			if (Netgame.Instagib)	// jinx 01-23-13 instagib
			{
				Laser_player_fire( objp, weapon_num, 0, 1, 0);
				total_shots++;
				break;
		...
		case VULCAN_INDEX: {
			...
			//if (d_rand() > 24576)
			//	make_sound = 1;
			int vulcan_spreadr, vulcan_spreadu;
			if (!promod)
			{
				vulcan_spreadr = d_rand()/8 - 32767/16;
				vulcan_spreadu = d_rand()/8 - 32767/16;
			}
			if (promod)		// jinx 02-15-13 promod
			{
				vulcan_spreadr = VULCAN_SPREAD_HORIZONTAL_PROMOD;
				vulcan_spreadu = VULCAN_SPREAD_VERTICAL_PROMOD;
			}
			Laser_player_fire_spread( objp, VULCAN_ID, 6, vulcan_spreadr, vulcan_spreadu, make_sound, 0);
			if (nfires > 1) {
				Laser_player_fire_spread( objp, VULCAN_ID, 6, vulcan_spreadr, vulcan_spreadu, 0, 0);
				if (nfires > 2) {
					Laser_player_fire_spread( objp, VULCAN_ID, 6, vulcan_spreadr, vulcan_spreadu, 0, 0);
				}
			}
		...
		if (promod)
		{
			if (flags & LASER_SPREADFIRE_TOGGLED) 
			{
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, F1_0/16*SPREADFIRE_SPREAD_PROMOD, 0, 0, 0);
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, -F1_0/16*SPREADFIRE_SPREAD_PROMOD, 0, 0, 0);
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, 0, 0, 1, 0);
			} 
			else 
			{
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, 0, F1_0/16*SPREADFIRE_SPREAD_PROMOD, 0, 0);
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, 0, -F1_0/16*SPREADFIRE_SPREAD_PROMOD, 0, 0);
				Laser_player_fire_spread( objp, SPREADFIRE_ID, 6, 0, 0, 1, 0);
			}
		}
		...
		case HELIX_INDEX: {
			...
			if (promod)
			{
				...
				{
					case 0: spreadr =  F1_0/16; spreadu = 0;       break; // Vertical
					case 1: spreadr =  F1_0/22; spreadu = F1_0/22; break; //  45   degrees
					case 2: spreadr =  0;       spreadu = F1_0/16; break; //  90   degrees
					case 3: spreadr = -F1_0/22; spreadu = F1_0/22; break; // 135   degrees
					case 4: spreadr =  F1_0/16; spreadu = 0;       break; // Vertical
					case 5: spreadr =  F1_0/22; spreadu = F1_0/22; break; //  45   degrees
					case 6: spreadr =  0;       spreadu = F1_0/16; break; //  90   degrees
					case 7: spreadr = -F1_0/22; spreadu = F1_0/22; break; // 135   degrees
					default:
						Error("Invalid helix_orientation value %x\n",helix_orient);
				}
				Laser_player_fire_spread( objp, HELIX_ID, 6,  0,  0, 1, 0);
				Laser_player_fire_spread( objp, HELIX_ID, 6,  spreadr*HELIX_BLOBS_2_PROMOD,  spreadu*HELIX_BLOBS_2_PROMOD, 0, 0);
				Laser_player_fire_spread( objp, HELIX_ID, 6, -spreadr*HELIX_BLOBS_2_PROMOD, -spreadu*HELIX_BLOBS_2_PROMOD, 0, 0);
				Laser_player_fire_spread( objp, HELIX_ID, 6,  spreadr*HELIX_BLOBS_3_PROMOD,  spreadu*HELIX_BLOBS_3_PROMOD, 0, 0);
				Laser_player_fire_spread( objp, HELIX_ID, 6, -spreadr*HELIX_BLOBS_3_PROMOD, -spreadu*HELIX_BLOBS_3_PROMOD, 0, 0);
			}
						break;
		}
...
void create_smart_children(object *objp, int num_smart_children)
{
	...
	{
		if (objp->id == SMART_ID)
			num_smart_children = NUM_SMART_CHILDREN_PROMOD;
		if (objp->id == EARTHSHAKER_ID)
			num_smart_children = NUM_EARTHSHAKER_CHILDREN_PROMOD;
		if (objp->id == SUPERPROX_ID)
			num_smart_children = NUM_SMARTMINE_CHILDREN_PROMOD;
	}
