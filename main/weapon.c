...
fix	primary_switch_times[10] = {LASER_SWITCH, VULCAN_SWITCH, SPREAD_SWITCH, PLASMA_SWITCH, FUSION_SWITCH, UNUSED_SWITCH, GAUSS_SWITCH, HELIX_SWITCH, PHOENIX_SWITCH, OMEGA_SWITCH};		// jinx 02-13-13 promod
fix secondary_switch_times[10] = {CONC_SWITCH, HOMER_SWITCH, PROX_SWITCH, SMART_SWITCH, MEGA_SWITCH, FLASH_SWITCH, GUIDED_SWITCH, SMINE_SWITCH, MERC_SWITCH, SHAKER_SWITCH};// jinx 02-13-13 promod
...
void select_weapon(int weapon_num, int secondary_flag, int print_message, int wait_for_rearm)
{
	...
	if (promod)		// jinx 02-13-13 promod
	{
		PROMOD_REARM_TIME = 0;
	}
	...
	if (!secondary_flag) {
		if (Primary_weapon != weapon_num) {
			...
			if (wait_for_rearm)
			{
				if (promod)		// jinx 02-13-13 promod
				{
					PROMOD_REARM_TIME += primary_switch_times[Primary_weapon];
					PROMOD_REARM_TIME += primary_switch_times[weapon_num];
					Next_laser_fire_time = GameTime64 + PROMOD_REARM_TIME;
				}
				else
		...
		if (Secondary_weapon != weapon_num) {
			...
			if (wait_for_rearm)
			{
				if (promod)		// jinx 02-13-13 promod
				{
					PROMOD_REARM_TIME += secondary_switch_times[Secondary_weapon];
					PROMOD_REARM_TIME += secondary_switch_times[weapon_num];
					Next_missile_fire_time = GameTime64 + PROMOD_REARM_TIME;
				}
				else
