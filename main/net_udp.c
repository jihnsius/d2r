...
void net_udp_send_game_info(struct _sockaddr sender_addr, ubyte info_upid)
{
	...
	else
	{
		...
		PUT_INTEL_INT(buf + len, Netgame.Descent1);						len += 4;		// jinx 02-02-13 d1
...
void net_udp_process_game_info(ubyte *data, int data_len, struct _sockaddr game_addr, int lite_info)
{
	...
	else
	{
		...
		Netgame.Descent1 = GET_INTEL_INT(&(data[len]));						len += 4;		// jinx 02-02-13 d1
...
static int opt_AutoReturnTime,opt_RespawnTime,opt_FlagInHome,opt_FlagIndicator,opt_BombIndicator,opt_d1,opt_Instagib;
...
menu:
	...
	Netgame.Descent1=m[opt_d1].value;		// jinx 02-02-13 d1
...
void net_udp_read_sync_packet( ubyte * data, int data_len, struct _sockaddr sender_addr )
{
	...
	if (!Network_rejoined)
	{
		...
		{
			if (((Game_mode & GM_CAPTURE) && (ctfc)) || assault)
			{
				if (first_spawns)
					NewPlayer = i;			
			...
			if (!(Game_mode & GM_CAPTURE) && !assault)
			{
