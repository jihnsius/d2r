...
void multi_prep_level(void)
{
	...
	for (i=0; i<=Highest_object_index; i++)
	{
		if (Netgame.Instagib && (Objects[i].type == OBJ_POWERUP) && (!(Objects[i].id == POW_AFTERBURNER) && !(Objects[i].id == POW_CLOAK) && !(Objects[i].id == POW_INVULNERABILITY) && !(Objects[i].id == POW_HEADLIGHT) && !(Objects[i].id == POW_FLAG_RED) && !(Objects[i].id == POW_FLAG_BLUE)))
		{
		...
		if (Objects[i].type == OBJ_POWERUP)
		{
			...
			if (Netgame.Descent1)		// jinx 02-16-13 d1
				bash_to_first(i,"whatever");
