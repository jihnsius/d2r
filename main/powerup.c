...
int do_powerup(object *obj)
{
	...
	if ((Game_mode & GM_MULTI) && !Netgame.Instagib)
	{
		/*
	...
	switch (obj->id) {
		...
		if (!Netgame.Instagib)
		{
			if ((!Netgame.Descent1 && (Players[Player_num].laser_level >= MAX_SUPER_LASER_LEVEL)) || (Netgame.Descent1 && (Players[Player_num].laser_level >= MAX_SUPER_LASER_LEVEL_D1))) 
			{
				if (!Netgame.Descent1)
					Players[Player_num].laser_level = MAX_SUPER_LASER_LEVEL;
				if (Netgame.Descent1)
					Players[Player_num].laser_level = MAX_SUPER_LASER_LEVEL_D1;
		...
		extern void multi_send_return_bonus(char pnum);
		...
		case POW_FLAG_RED:
		{
			if (Game_mode & GM_CAPTURE)			
			{
				...
				if (get_team(Player_num) == TEAM_RED)
				{
					...
					{
						if ((Segment2s[r].special == SEGMENT_IS_GOAL_RED) && segment_contains_object(OBJ_POWERUP, POW_FLAG_RED, r)) break; 
						if (r == Highest_segment_index+1) 
						{
							obj->flags |= OF_SHOULD_BE_DEAD;
							multi_send_remobj(obj-Objects);
							multi_send_return_flag(obj, Player_num);
							multi_send_flag_disappearance(obj);
							//object_create_explosion(obj->segnum, &obj->pos, F1_0*7/2, VCLIP_POWERUP_DISAPPEARANCE );
							break;
						}
					}
		...
		case POW_FLAG_BLUE:
		{
			if (Game_mode & GM_CAPTURE)			
			{
				...
				if (get_team(Player_num) == TEAM_BLUE)
				{
					...
					{
						if ((Segment2s[b].special == SEGMENT_IS_GOAL_BLUE) && segment_contains_object(OBJ_POWERUP, POW_FLAG_BLUE, b))	break;
						if (b == Highest_segment_index+1)
						{
							obj->flags |= OF_SHOULD_BE_DEAD;
							multi_send_remobj(obj-Objects);
							multi_send_return_flag(obj, Player_num);
