...
void do_afterburner_stuff(void)
{
	...
	if (!(Players[Player_num].flags & PLAYER_FLAGS_AFTERBURNER)) // jinx 01-25-13 spec
		Afterburner_charge = 0;

	if (Endlevel_sequence || Player_is_dead || Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)		// jinx 01-25-13 spec
	{
...
void GameProcessFrame(void)
{
	...
	if ((Game_mode & GM_MULTI) && (Players[Player_num].connected == CONNECT_PLAYING))
	{
		if ( Endlevel_sequence || ((Control_center_destroyed) && (Countdown_seconds_left <= 1)) || // close menus when end of level...
			(Automap_active && ((Player_is_dead != player_was_dead) || (Players[Player_num].shields<=0 && player_shields>0))) )
				if (settings_leave_menu_when_hit)
					game_leave_menus();	// jinx 02-05-13 don't leave menus when hit if player doesn't want it
	}
}
