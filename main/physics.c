...
fix64 GRAVITY_DEFAULT = 1;
fix64 PLAYER_GRAVITY = 10;
fix64 FLARE_GRAVITY = 8000000;
fix64 GAUSS_GRAVITY = 152000000;
fix64 MEGA_GRAVITY = 6000000;
#define TERMINAL_VELOCITY	F1_0*10
...
void do_gravity(object * obj)
{
	...
	int fall_speed;
	fall_speed = GRAVITY_DEFAULT;
	if (obj->type == OBJ_PLAYER)
		fall_speed = PLAYER_GRAVITY;
	if (obj->type == OBJ_WEAPON)
	{
		if (obj->id == FLARE_ID)
			fall_speed = FLARE_GRAVITY;
		if (obj->id == GAUSS_ID)
			fall_speed = GAUSS_GRAVITY;
		if (obj->id == VULCAN_ID)
			fall_speed = GAUSS_GRAVITY;
		if (obj->id == MEGA_ID || obj->id == EARTHSHAKER_ID)
...
void do_physics_sim(object *obj)
{
	...
	do_gravity(obj);		// jinx 02-16-13 gravity
					...
					if (!forcefield_bounce && (obj->mtype.phys_info.flags & PF_STICK)) {		//stop moving
						...
						if (forcefield_bounce || (obj->mtype.phys_info.flags & PF_BOUNCE)) {		//bounce off wall
							...
							if ( obj->mtype.phys_info.flags & PF_BOUNCES_TWICE) 
							{
								...
								if (promod)		// jinx 02-14-13 promod
									obj->lifeleft /= PHOENIX_LIFE_DIVISOR;
							}
